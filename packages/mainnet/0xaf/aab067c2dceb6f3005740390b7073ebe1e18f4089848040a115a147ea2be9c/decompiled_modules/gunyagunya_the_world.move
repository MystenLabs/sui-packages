module 0xafaab067c2dceb6f3005740390b7073ebe1e18f4089848040a115a147ea2be9c::gunyagunya_the_world {
    struct GUNYAGUNYA_THE_WORLD has drop {
        dummy_field: bool,
    }
    ...
    fun init(arg0: GUNYAGUNYA_THE_WORLD, arg1: &mut 0x2::tx_context::TxContext) {
        let v0 = 0x2::package::claim<GUNYAGUNYA_THE_WORLD>(arg0, arg1);
        let v1 = 0x2::display::new<Nft>(&v0, arg1);
        0x2::display::add<Nft>(&mut v1, 0x1::string::utf8(b"collection_name"), 0x1::string::utf8(b"gunyagunya the world"));
        0x2::display::add<Nft>(&mut v1, 0x1::string::utf8(b"collection_description"), 0x1::string::utf8(b"In this world, everything is divided into four distinct colors, each following a unique and mysterious pattern. These colors are not chosen at random-they are woven into the very fabric of reality, shaping destinies, boundaries, and the flow of power. The interplay of these four hues creates a dynamic balance, where every element and every being finds its place according to the secret logic of the chromatic order. This is a realm where color is more than appearance; it is the code that governs existence itself."));
        0x2::display::add<Nft>(&mut v1, 0x1::string::utf8(b"collection_media_url"), 0x1::string::utf8(b"ipfs://bafybeihbqzlf5gt45jrrydfkppcmlmo24gq2j4k5nydo7opg4ntf4laosm"));
