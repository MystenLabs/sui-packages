module 0x2a1aacd72da22e0472780fb4ef3fd000620169faf66541f7745802316ee23075::pool_rewards {
    ...
    public fun claim_custom_token_rewards<T0>(arg0: &PoolSystem, arg1: &mut CustomTokenPool<T0>, arg2: &0x2::clock::Clock, arg3: &mut 0x2::tx_context::TxContext) {
        ...
        let v5 = arg1.end_time - arg1.start_time;
        let v6 = if (v1 > arg1.end_time) {
            arg1.end_time - arg1.start_time
        } else {
            v1 - arg1.start_time
        };
        let v7 = if (v4 == 0) {
        ...
        let v8 = if (v7) {
            ...
            let v9 = (v3 as u128) * (v6 as u128) * (0x2::balance::value<T0>(&arg1.token_reward_pool) as u128) / (v4 as u128) * (v5 as u128);
        ...
        if (v8 > 0 && 0x2::balance::value<T0>(&arg1.token_reward_pool) >= v8) {
            0x2::table::borrow_mut<address, UserStakeInfo>(&mut arg1.user_stakes, v0).last_reward_claim = v1;
            0x2::transfer::public_transfer<0x2::coin::Coin<T0>>(0x2::coin::from_balance<T0>(0x2::balance::split<T0>(&mut arg1.token_reward_pool, v8), arg3), v0);
            let v10 = CustomTokenRewardsClaimed{
                pool_id    : arg1.pool_id,
                user       : v0,
                token_type : 0x1::string::utf8(0x1::ascii::into_bytes(0x1::type_name::into_string(0x1::type_name::with_defining_ids<T0>()))),
                amount     : v8,
            };
            0x2::event::emit<CustomTokenRewardsClaimed>(v10);
        };
    }
