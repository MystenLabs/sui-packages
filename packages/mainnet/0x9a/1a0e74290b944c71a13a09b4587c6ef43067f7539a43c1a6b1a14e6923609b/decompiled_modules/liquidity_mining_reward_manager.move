module 0x9a1a0e74290b944c71a13a09b4587c6ef43067f7539a43c1a6b1a14e6923609b::liquidity_mining_reward_manager {
    ...
    struct PoolReward has store, key {
        ...
        end_time_ms: u64,
        total_rewards: u64,
        allocated_rewards: 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::Decimal,
        cumulative_rewards_per_share: 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::Decimal,
    ...
    struct ObligationReward has store {
        ...
        earned_rewards: 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::Decimal,
        cumulative_rewards_per_share: 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::Decimal,
    }
    ...
    public(friend) fun add_pool_reward<T0>(arg0: &mut PoolRewardManager, arg1: 0x2::balance::Balance<T0>, arg2: u64, arg3: u64, arg4: &0x2::clock::Clock, arg5: &mut 0x2::tx_context::TxContext) {
        assert!(arg2 >= 0x2::clock::timestamp_ms(arg4), 0x9a1a0e74290b944c71a13a09b4587c6ef43067f7539a43c1a6b1a14e6923609b::error::liquidity_mining_invalid_start_time());
        assert!(arg3 - arg2 >= 3600000, 0x9a1a0e74290b944c71a13a09b4587c6ef43067f7539a43c1a6b1a14e6923609b::error::liquidity_mining_invalid_time());
        ...
        let v2 = PoolReward{
            ...
            allocated_rewards              : 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::from(0),
            cumulative_rewards_per_share   : 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::from(0),
            num_obligation_reward_managers : 0,
            additional_fields              : v0,
        };
        let v3 = find_available_index(arg0);
        assert!(v3 < 50, 0x9a1a0e74290b944c71a13a09b4587c6ef43067f7539a43c1a6b1a14e6923609b::error::liquidity_mining_max_concurrent_pool_rewards_violated());
    ...
    public(friend) fun cancel_pool_reward<T0>(arg0: &mut PoolRewardManager, arg1: u64, arg2: &0x2::clock::Clock) : 0x2::balance::Balance<T0> {
        ...
        v0.total_rewards = 0;
        let v1 = RewardBalance<T0>{dummy_field: false};
        0x2::balance::split<T0>(0x2::bag::borrow_mut<RewardBalance<T0>, 0x2::balance::Balance<T0>>(&mut v0.additional_fields, v1), 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::floor(0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::sub(0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::from(v0.total_rewards), v0.allocated_rewards)))
    }
    ...
    public(friend) fun claim_rewards<T0>(arg0: &mut PoolRewardManager, arg1: 0x2::object::ID, arg2: &0x2::clock::Clock, arg3: u64) : 0x2::balance::Balance<T0> {
        ...
        assert!(v0.coin_type == 0x1::type_name::with_defining_ids<T0>(), 0x9a1a0e74290b944c71a13a09b4587c6ef43067f7539a43c1a6b1a14e6923609b::error::liquidity_mining_invalid_type());
        ...
        assert!(0x1::option::is_some<ObligationReward>(v1), 0x9a1a0e74290b944c71a13a09b4587c6ef43067f7539a43c1a6b1a14e6923609b::error::liquidity_mining_no_reward_to_claim());
        let v2 = 0x1::option::borrow_mut<ObligationReward>(v1);
        let v3 = 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::floor(v2.earned_rewards);
        v2.earned_rewards = 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::sub(v2.earned_rewards, 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::from(v3));
    ...
    public(friend) fun close_pool_reward<T0>(arg0: &mut PoolRewardManager, arg1: u64, arg2: &0x2::clock::Clock) : 0x2::balance::Balance<T0> {
        ...
        assert!(0x2::clock::timestamp_ms(arg2) >= v4, 0x9a1a0e74290b944c71a13a09b4587c6ef43067f7539a43c1a6b1a14e6923609b::error::liquidity_mining_pool_reward_period_not_over());
        assert!(v8 == 0, 0x9a1a0e74290b944c71a13a09b4587c6ef43067f7539a43c1a6b1a14e6923609b::error::liquidity_mining_not_all_rewards_claimed());
            ...
            if (0x1::option::is_none<ObligationReward>(v5)) {
                if (v0.last_update_time_ms <= v4.end_time_ms) {
                    let v6 = if (v0.last_update_time_ms <= v4.start_time_ms) {
                        0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::mul(v4.cumulative_rewards_per_share, 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::from(v0.share))
                    } else {
                        0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::from(0)
                    };
                ...
                v8.earned_rewards = 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::add(v8.earned_rewards, 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::mul(0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::sub(v4.cumulative_rewards_per_share, v8.cumulative_rewards_per_share), 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::from(v0.share)));
    ...
    fun update_pool_reward_manager(arg0: &mut PoolRewardManager, arg1: &0x2::clock::Clock) {
        ...
        while (v1 < 0x1::vector::length<0x1::option::Option<PoolReward>>(&arg0.pool_rewards)) {
            ...
            let v4 = 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::div(0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::mul(0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::from(v3.total_rewards), 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::from(0x1::u64::min(v0, v3.end_time_ms) - 0x1::u64::max(v3.start_time_ms, arg0.last_update_time_ms))), 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::from(v3.end_time_ms - v3.start_time_ms));
            v3.allocated_rewards = 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::add(v3.allocated_rewards, v4);
            v3.cumulative_rewards_per_share = 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::add(v3.cumulative_rewards_per_share, 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::div(v4, 0x7d779e7ae65c4d9e296e77a6bc3e80c0d364a98a4314f75acc3f47d676db939a::float::from(arg0.total_shares)));
            v1 = v1 + 1;
        };
        arg0.last_update_time_ms = v0;
    }
