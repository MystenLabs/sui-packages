module 0x9500cf3ccdfd8920999ddb0dccc40a44edae65665ebe96866f8b6f2d1d70a010::stonk {
    struct STONK has drop {
        dummy_field: bool,
    }

    fun init(arg0: STONK, arg1: &mut 0x2::tx_context::TxContext) {
        let (v0, v1) = 0x2::coin_registry::new_currency_with_otw<STONK>(arg0, 9, untag(b"SSTONK"), untag(b"NStonk"), untag(b"D"), untag(b"Ihttps://silver-perfect-wren-754.mypinata.cloud/ipfs/bafkreicawvd7i5jqex732jefpzfkkbxshci7xm5yti6fcskgffnou6nawa"), arg1);
        let v2 = v1;
        let v3 = v0;
        0x2::coin_registry::make_supply_burn_only_init<STONK>(&mut v3, v2);
        0x2::coin_registry::finalize_and_delete_metadata_cap<STONK>(v3, arg1);
        0x2::transfer::public_transfer<0x2::coin::Coin<STONK>>(0x2::coin::mint<STONK>(&mut v2, 1000000000000000000, arg1), 0x2::tx_context::sender(arg1));
    }

    fun untag(arg0: vector<u8>) : 0x1::string::String {
        0x1::vector::remove<u8>(&mut arg0, 0);
        0x1::string::utf8(arg0)
    }

    // decompiled from Move bytecode v7
}

