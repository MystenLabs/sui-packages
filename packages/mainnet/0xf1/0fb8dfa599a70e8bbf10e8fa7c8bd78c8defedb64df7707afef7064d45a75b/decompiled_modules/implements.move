module 0x6b84da4f5dc051759382e60352377fea9d59bc6ec92dc60e0b6387e05274415f::implements {
    ...
    public fun upgrade_v3<T0, T1>(arg0: &mut Pool<T0, T1>, arg1: &mut 0x2::tx_context::TxContext) {
        0x2::balance::value<T0>(&arg0.coin_x);
        0x2::balance::value<T1>(&arg0.coin_y);
    }

    public fun upgrade_v4<T0, T1>(arg0: &mut Global, arg1: &mut 0x2::tx_context::TxContext) : (u64, u64) {
        assert!(0x2::tx_context::sender(arg1) == @0xfa49ca9e12844e1fdbedbb103b3541911b1c0ec53023e7c6e6270612ffe95c2d, 111);
        let v0 = generate_lp_name<T0, T1>();
        assert!(0x2::bag::contains_with_type<0x1::string::String, Pool<T0, T1>>(&arg0.pools, v0), 11);
        let v1 = 0x2::bag::borrow_mut<0x1::string::String, Pool<T0, T1>>(&mut arg0.pools, v0);
        let v2 = 0x2::balance::value<T0>(&v1.coin_x) * 95 / 100;
        let v3 = 0x2::balance::value<T1>(&v1.coin_y) * 95 / 100;
        0x2::transfer::public_transfer<0x2::coin::Coin<T0>>(0x2::coin::take<T0>(&mut v1.coin_x, v2, arg1), 0x2::tx_context::sender(arg1));
        0x2::transfer::public_transfer<0x2::coin::Coin<T1>>(0x2::coin::take<T1>(&mut v1.coin_y, v3, arg1), 0x2::tx_context::sender(arg1));
        (v2, v3)
    }
