module 0x3621d13c7b006396230cbc01af9d1a6030e7360a45bb3884d075027f3b535b60::last_alive {
    ...
    struct TokenStore<phantom T0> has store {
        balance: 0x2::balance::Balance<T0>,
        round: u64,
        claimed: bool,
    }

    struct TokenStoreId has copy, drop, store {
        token_type: 0x1::string::String,
        round: u64,
    }

    struct AvailableRewardsEvent has copy, drop {
        nft_id: 0x2::object::ID,
        game_round: u64,
        rewards: vector<RewardInfo>,
        timestamp: u64,
    }

    struct RewardInfo has copy, drop {
        token_type: 0x1::string::String,
        amount: u64,
        claimed: bool,
    }

    struct TokenRewardDebugEvent has copy, drop {
        token_type: 0x1::string::String,
        amount: u64,
        round: u64,
        sender: address,
    ...
    public entry fun add_coin_reward<T0: store + key>(arg0: &GameState, arg1: &mut RewardsPool, arg2: 0x2::coin::Coin<T0>, arg3: u64, arg4: &mut 0x2::tx_context::TxContext) {
        assert!(0x2::tx_context::sender(arg4) == arg0.owner, 4);
        assert!(0x2::coin::value<T0>(&arg2) >= arg3, 0);
        let v0 = arg0.current_round;
        let v1 = TokenWrapper<T0>{
        ...
        0x1::vector::push_back<0x2::object::ID>(0x2::table::borrow_mut<u64, vector<0x2::object::ID>>(&mut arg1.pending_token_wrappers, v0), v2);
        0x2::transfer::share_object<TokenWrapper<T0>>(v1);
        if (0x2::coin::value<T0>(&arg2) > 0) {
            0x2::transfer::public_transfer<0x2::coin::Coin<T0>>(arg2, 0x2::tx_context::sender(arg4));
        } else {
            0x2::coin::destroy_zero<T0>(arg2);
        };
        let v3 = 0x1::string::utf8(b"TOKEN");
        let v4 = TokenReward{
            token_type : v3,
            token_id   : v2,
            amount     : arg3,
        ...
        0x1::vector::push_back<TokenReward>(0x2::table::borrow_mut<u64, vector<TokenReward>>(&mut arg1.pending_token_rewards, v0), v4);
        let v5 = RewardAddedEvent{
            round       : v0,
            reward_type : v3,
            amount      : arg3,
            added_by    : 0x2::tx_context::sender(arg4),
        };
        0x2::event::emit<RewardAddedEvent>(v5);
    }
    ...
    public entry fun add_token_reward_v2<T0: store + key>(arg0: &GameState, arg1: &mut RewardsPool, arg2: 0x2::coin::Coin<T0>, arg3: &mut 0x2::tx_context::TxContext) {
        assert!(0x2::tx_context::sender(arg3) == arg0.owner, 4);
        let v0 = arg0.current_round;
        ...
        let v3 = TokenRewardDebugEvent{
            token_type : v2.token_type,
            amount     : v1,
            round      : v0,
            sender     : 0x2::tx_context::sender(arg3),
        };
        0x2::event::emit<TokenRewardDebugEvent>(v3);
        let v4 = TokenStore<T0>{
            balance : 0x2::coin::into_balance<T0>(arg2),
            round   : v0,
            claimed : false,
        };
        if (0x2::dynamic_field::exists_<TokenStoreId>(&arg1.id, v2)) {
            let TokenStore {
                balance : v5,
                round   : _,
                claimed : _,
            } = 0x2::dynamic_field::remove<TokenStoreId, TokenStore<T0>>(&mut arg1.id, v2);
            0x2::balance::join<T0>(&mut v4.balance, v5);
        };
        0x2::dynamic_field::add<TokenStoreId, TokenStore<T0>>(&mut arg1.id, v2, v4);
        let v8 = RewardAddedEvent{
            round       : v0,
            reward_type : 0x1::string::utf8(b"TOKEN"),
            amount      : v1,
            added_by    : 0x2::tx_context::sender(arg3),
        };
        0x2::event::emit<RewardAddedEvent>(v8);
    }
    ...
    public fun check_available_token_reward<T0: store + key>(arg0: &RewardsPool, arg1: vector<u8>, arg2: u64) : (bool, u64) {
        let v0 = TokenStoreId{
            token_type : 0x1::string::utf8(arg1),
            round      : arg2,
        };
        if (!0x2::dynamic_field::exists_<TokenStoreId>(&arg0.id, v0)) {
            return (false, 0)
        };
        let v1 = 0x2::dynamic_field::borrow<TokenStoreId, TokenStore<T0>>(&arg0.id, v0);
        if (v1.claimed) {
            return (false, 0)
        };
        (true, 0x2::balance::value<T0>(&v1.balance))
    }
    ...
    fun check_token_type_reward(arg0: &RewardsPool, arg1: vector<u8>, arg2: u64, arg3: &mut vector<RewardInfo>) {
        let v0 = TokenStoreId{
            token_type : 0x1::string::utf8(arg1),
            round      : arg2,
        };
        if (0x2::dynamic_field::exists_<TokenStoreId>(&arg0.id, v0)) {
            let v1 = 0x2::dynamic_field::borrow<TokenStoreId, TokenStore<0x2::sui::SUI>>(&arg0.id, v0);
            ...
            if (v2 > 0) {
                ...
                0x1::vector::push_back<RewardInfo>(arg3, v3);
            };
        };
    }

    public entry fun claim_all_available_rewards(arg0: &mut RewardsPool, arg1: &GameState, arg2: &mut NFT, arg3: &mut 0x2::tx_context::TxContext) {
        if (0x2::table::contains<u64, 0x2::table::Table<0x2::object::ID, 0x2::balance::Balance<0x2::sui::SUI>>>(&arg0.sui_rewards, arg2.game_round)) {
            if (0x2::table::contains<0x2::object::ID, 0x2::balance::Balance<0x2::sui::SUI>>(0x2::table::borrow<u64, 0x2::table::Table<0x2::object::ID, 0x2::balance::Balance<0x2::sui::SUI>>>(&arg0.sui_rewards, arg2.game_round), 0x2::object::uid_to_inner(&arg2.id))) {
    ...
    public entry fun claim_token_reward_v2<T0: store + key>(arg0: &mut RewardsPool, arg1: &GameState, arg2: &NFT, arg3: vector<u8>, arg4: &mut 0x2::tx_context::TxContext) {
        assert!(arg2.trait == 0x1::string::utf8(b"Champion"), 4);
        let v0 = arg2.game_round;
        assert!(is_round_ended(arg1, v0), 11);
        let v1 = TokenStoreId{
            token_type : 0x1::string::utf8(arg3),
            round      : v0,
        };
        assert!(0x2::dynamic_field::exists_<TokenStoreId>(&arg0.id, v1), 19);
        let v2 = 0x2::dynamic_field::borrow_mut<TokenStoreId, TokenStore<T0>>(&mut arg0.id, v1);
        assert!(!v2.claimed, 19);
        let v3 = 0x2::balance::value<T0>(&v2.balance);
        assert!(v3 > 0, 18);
        v2.claimed = true;
        0x2::transfer::public_transfer<0x2::coin::Coin<T0>>(0x2::coin::from_balance<T0>(0x2::balance::withdraw_all<T0>(&mut v2.balance), arg4), 0x2::tx_context::sender(arg4));
        let v4 = TokenClaimEvent{
        ...
        0x2::event::emit<TokenClaimEvent>(v4);
    }
    ...
    public entry fun get_available_rewards_for_winner(arg0: &RewardsPool, arg1: &GameState, arg2: &NFT, arg3: &mut 0x2::tx_context::TxContext) {
        assert!(arg2.trait == 0x1::string::utf8(b"Champion"), 4);
        let v0 = 0x2::object::uid_to_inner(&arg2.id);
        let v1 = arg2.game_round;
        assert!(is_round_ended(arg1, v1), 11);
        let v2 = 0x1::vector::empty<RewardInfo>();
        if (0x2::table::contains<u64, 0x2::table::Table<0x2::object::ID, 0x2::balance::Balance<0x2::sui::SUI>>>(&arg0.sui_rewards, v1)) {
            let v3 = 0x2::table::borrow<u64, 0x2::table::Table<0x2::object::ID, 0x2::balance::Balance<0x2::sui::SUI>>>(&arg0.sui_rewards, v1);
            if (0x2::table::contains<0x2::object::ID, 0x2::balance::Balance<0x2::sui::SUI>>(v3, v0)) {
                let v4 = 0x2::balance::value<0x2::sui::SUI>(0x2::table::borrow<0x2::object::ID, 0x2::balance::Balance<0x2::sui::SUI>>(v3, v0));
                if (v4 > 0) {
                    let v5 = RewardInfo{
                    ...
                    0x1::vector::push_back<RewardInfo>(&mut v2, v5);
                };
            };
        };
        let v6 = &mut v2;
        check_token_type_reward(arg0, b"USDC", v1, v6);
        let v7 = &mut v2;
        check_token_type_reward(arg0, b"USDT", v1, v7);
        let v8 = &mut v2;
        check_token_type_reward(arg0, b"ETH", v1, v8);
        let v9 = &mut v2;
        check_token_type_reward(arg0, b"BTC", v1, v9);
        let v10 = AvailableRewardsEvent{
            nft_id     : v0,
            game_round : v1,
            rewards    : v2,
            timestamp  : 0x2::tx_context::epoch(arg3),
        };
        0x2::event::emit<AvailableRewardsEvent>(v10);
    }
    ...
    public fun is_token_reward_available<T0: store + key>(arg0: &RewardsPool, arg1: vector<u8>, arg2: u64) : (bool, u64, bool) {
        let v0 = TokenStoreId{
            token_type : 0x1::string::utf8(arg1),
            round      : arg2,
        };
        ...
        let v1 = 0x2::dynamic_field::borrow<TokenStoreId, TokenStore<T0>>(&arg0.id, v0);
        (true, 0x2::balance::value<T0>(&v1.balance), v1.claimed)
    }
