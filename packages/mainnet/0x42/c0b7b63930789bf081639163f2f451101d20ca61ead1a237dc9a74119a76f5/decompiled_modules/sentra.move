module 0x42c0b7b63930789bf081639163f2f451101d20ca61ead1a237dc9a74119a76f5::sentra {
    ...
    public entry fun complete_yield_withdrawal_with_redeemed_coin<T0, T1>(arg0: YieldLock<T1>, arg1: 0x2::coin::Coin<T0>, arg2: &mut Platform, arg3: &mut UserRegistry, arg4: &0x2::clock::Clock, arg5: &mut 0x2::tx_context::TxContext) {
        ...
        let (v16, v17) = if (v12 >= v3 + v4) {
            (v14, 0)
        } else {
            let v18 = safe_mul_div(v5, 200, 10000);
            let v19 = if (v18 > v13) {
                v13
            } else {
                v18
        ...
        let (v21, v22) = if (v15 > 0) {
            let v23 = safe_mul_div(v15, 3000, 10000);
            (v23, v15 - v23)
        } else {
            (0, 0)
        };
        let v24 = v16 + v22;
        let v25 = v17 + v21;
        assert!(v24 + v25 <= 0x2::coin::value<T0>(&arg1), 5);
        if (v24 > 0) {
            0x2::transfer::public_transfer<0x2::coin::Coin<T0>>(0x2::coin::split<T0>(&mut arg1, v24, arg5), v0);
        };
        if (v25 > 0) {
            let v26 = 0x2::coin::into_balance<T0>(0x2::coin::split<T0>(&mut arg1, v25, arg5));
            if (v17 > 0) {
                0x2::balance::join<T0>(0x2::bag::borrow_mut<0x1::type_name::TypeName, 0x2::balance::Balance<T0>>(&mut arg2.fees, v10), 0x2::balance::split<T0>(&mut v26, v17));
            };
            if (v21 > 0) {
                0x2::balance::join<T0>(0x2::bag::borrow_mut<0x1::type_name::TypeName, 0x2::balance::Balance<T0>>(&mut arg2.yield_fees, v10), v26);
            } else {
        ...
        if (0x2::vec_map::contains<address, vector<0x2::object::ID>>(&arg3.yield_locks, &v0)) {
            let v27 = 0x2::vec_map::get_mut<address, vector<0x2::object::ID>>(&mut arg3.yield_locks, &v0);
            remove_lock_id(v27, v11);
            if (0x1::vector::is_empty<0x2::object::ID>(v27)) {
                let (_, _) = 0x2::vec_map::remove<address, vector<0x2::object::ID>>(&mut arg3.yield_locks, &v0);
            };
        };
        update_yield_tvl<T0>(arg2, v5, false);
        let v30 = &mut arg2.global_yield_lock_list;
        remove_lock_id(v30, v11);
        let v31 = YieldLockWithdrawn{
            owner               : v0,
            principal_withdrawn : v16,
            yield_earned        : v15,
            platform_yield_fee  : v21,
            user_yield_amount   : v22,
            withdrawn_time      : v12,
            coin_type           : v10,
        };
        0x2::event::emit<YieldLockWithdrawn>(v31);
    ...
    public entry fun create_yield_lock<T0, T1>(arg0: &mut Platform, arg1: &mut UserRegistry, arg2: 0x2::coin::Coin<T1>, arg3: u64, arg4: &0x2::clock::Clock, arg5: &mut 0x2::tx_context::TxContext) {
        ...
        0x1::vector::push_back<0x2::object::ID>(&mut arg0.global_yield_lock_list, v8);
