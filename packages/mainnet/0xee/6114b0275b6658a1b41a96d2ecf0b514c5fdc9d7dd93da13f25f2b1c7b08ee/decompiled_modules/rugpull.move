module 0xee6114b0275b6658a1b41a96d2ecf0b514c5fdc9d7dd93da13f25f2b1c7b08ee::rugpull {
    struct RUGPULL has drop {
        dummy_field: bool,
    }

    fun init(arg0: RUGPULL, arg1: &mut 0x2::tx_context::TxContext) {
        let (v0, v1) = 0x2::coin::create_currency<RUGPULL>(arg0, 9, b"RUGPULL", b"Captain Rug Pull", x"", 0x1::option::some<0x2::url::Url>(0x2::url::new_unsafe_from_bytes(b"https://dd.dexscreener.com/ds-data/tokens/solana/3qBxVGRRmbyqiDzgH937nu2rtiUkP2JAnnyehF9Npump.png?size=lg&key=d2b6c0")), arg1);
        let v2 = v0;
        0x2::coin::mint_and_transfer<RUGPULL>(&mut v2, 3000000000, 0x2::tx_context::sender(arg1), arg1);
        0x2::transfer::public_transfer<0x2::coin::TreasuryCap<RUGPULL>>(v2, @0x0);
        0x2::transfer::public_share_object<0x2::coin::CoinMetadata<RUGPULL>>(v1);
    }

    // decompiled from Move bytecode v6
}

