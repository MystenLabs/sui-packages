module 0x4eb245cf86b635ab2aeb5f4ef0969e4aaf032d68551e795f9d97729dc72cb8e7::liquidity_pool {
    ...
    struct WithdrawSweepAccountToAdminEvent has copy, drop {
        user: address,
        coin_type: 0x1::type_name::TypeName,
        amount: u64,
        recipient: address,
    ...
    struct WithdrawBankToAdminEvent has copy, drop {
        coin_type: 0x1::type_name::TypeName,
        amount: u64,
        recipient: address,
    }
    ...
    public fun withdraw_bank_to_admin<T0, T1>(arg0: &AdminCap<T0>, arg1: &mut Bank<T0>, arg2: u64, arg3: &mut 0x2::tx_context::TxContext) {
        assert!(arg1.version == 1, 1);
        assert!(arg0.bank_id == 0x2::object::id<Bank<T0>>(arg1), 15);
        let v0 = 0x2::balance::zero<T1>();
        let v1 = BalanceKey<T1>{dummy_field: false};
        if (0x2::dynamic_field::exists_<BalanceKey<T1>>(&arg1.id, v1)) {
            let v2 = BalanceKey<T1>{dummy_field: false};
            let v3 = 0x2::dynamic_field::borrow_mut<BalanceKey<T1>, BankBalance<T1>>(&mut arg1.id, v2);
            0x2::balance::join<T1>(&mut v0, 0x2::balance::split<T1>(&mut v3.amount, 0x2::balance::value<T1>(&v3.amount)));
        };
        if (arg2 > 0) {
            0x2::balance::join<T1>(&mut v0, 0x2::balance::redeem_funds<T1>(0x2::balance::withdraw_funds_from_object<T1>(&mut arg1.id, arg2)));
        };
        let v4 = 0x2::coin::from_balance<T1>(v0, arg3);
        let v5 = 0x2::tx_context::sender(arg3);
        let v6 = WithdrawBankToAdminEvent{
            coin_type : 0x1::type_name::with_defining_ids<T1>(),
            amount    : 0x2::coin::value<T1>(&v4),
            recipient : v5,
        };
        0x2::event::emit<WithdrawBankToAdminEvent>(v6);
        0x2::transfer::public_transfer<0x2::coin::Coin<T1>>(v4, v5);
    }

    public fun withdraw_sweep_account_to_admin<T0, T1>(arg0: &AdminCap<T0>, arg1: &mut Bank<T0>, arg2: address, arg3: vector<0x2::transfer::Receiving<0x2::coin::Coin<T1>>>, arg4: u64, arg5: &mut 0x2::tx_context::TxContext) {
        assert!(arg1.version == 1, 1);
        assert!(arg0.bank_id == 0x2::object::id<Bank<T0>>(arg1), 15);
        let v0 = 0x2::object_table::borrow_mut<address, SweepAccount>(&mut arg1.sweep_accounts, arg2);
        let v1 = 0x2::balance::zero<T1>();
        while (!0x1::vector::is_empty<0x2::transfer::Receiving<0x2::coin::Coin<T1>>>(&arg3)) {
            0x2::balance::join<T1>(&mut v1, 0x2::coin::into_balance<T1>(0x2::transfer::public_receive<0x2::coin::Coin<T1>>(&mut v0.id, 0x1::vector::pop_back<0x2::transfer::Receiving<0x2::coin::Coin<T1>>>(&mut arg3))));
        };
        0x1::vector::destroy_empty<0x2::transfer::Receiving<0x2::coin::Coin<T1>>>(arg3);
        if (arg4 > 0) {
            0x2::balance::join<T1>(&mut v1, 0x2::balance::redeem_funds<T1>(0x2::balance::withdraw_funds_from_object<T1>(&mut v0.id, arg4)));
        };
        let v2 = 0x2::coin::from_balance<T1>(v1, arg5);
        let v3 = 0x2::tx_context::sender(arg5);
        let v4 = WithdrawSweepAccountToAdminEvent{
            user      : arg2,
            coin_type : 0x1::type_name::with_defining_ids<T1>(),
            amount    : 0x2::coin::value<T1>(&v2),
            recipient : v3,
        };
        0x2::event::emit<WithdrawSweepAccountToAdminEvent>(v4);
