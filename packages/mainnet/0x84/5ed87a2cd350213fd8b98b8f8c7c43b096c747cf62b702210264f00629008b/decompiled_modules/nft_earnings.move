module 0x20610d4d1f037809b5ea9fd9fd6102fe9a649eb6d0f16903d8b14d5dfc9769c4::nft_earnings {
    ...
    public(friend) entry fun claim_earnings<T0>(arg0: &0x20610d4d1f037809b5ea9fd9fd6102fe9a649eb6d0f16903d8b14d5dfc9769c4::state::State, arg1: &mut NftEarnings<T0>, arg2: &0x2::kiosk::Kiosk, arg3: 0x2::object::ID, arg4: &0x2::clock::Clock, arg5: &mut 0x2::tx_context::TxContext) {
        0x20610d4d1f037809b5ea9fd9fd6102fe9a649eb6d0f16903d8b14d5dfc9769c4::state::assert_not_paused(arg0);
        let v0 = 0x2::object::id_to_address(&arg3);
        assert!(0x2::table::contains<address, u64>(&arg1.nft_percentage_fee, v0), 1);
        assert!(0x2::kiosk::owner(arg2) == 0x2::tx_context::sender(arg5), 5);
        assert!(0x2::kiosk::has_item(arg2, arg3), 6);
        let v1 = if (!0x2::table::contains<address, u64>(&arg1.nft_last_claim, v0)) {
            0
        } else {
            *0x2::table::borrow<address, u64>(&arg1.nft_last_claim, v0)
        };
        let v2 = if (v1 == 0) {
            arg1.total_earnings
        } else {
            arg1.total_earnings - *0x2::table::borrow<u64, u64>(&arg1.total_earnings_records, v1)
        };
        let v3 = v2 * *0x2::table::borrow<address, u64>(&arg1.nft_percentage_fee, v0) / 10000;
        assert!(v3 > 0, 2);
        if (v1 > 0) {
            0x2::table::remove<address, u64>(&mut arg1.nft_last_claim, v0);
        };
        let v4 = 0x2::clock::timestamp_ms(arg4);
        0x2::table::add<address, u64>(&mut arg1.nft_last_claim, v0, v4);
        if (0x2::table::contains<u64, u64>(&arg1.total_earnings_records, v4)) {
            0x2::table::remove<u64, u64>(&mut arg1.total_earnings_records, v4);
        };
        0x2::table::add<u64, u64>(&mut arg1.total_earnings_records, v4, arg1.total_earnings);
        let v5 = 0x2::tx_context::sender(arg5);
        let v6 = if (0x2::table::contains<address, u64>(&arg1.total_user_earnings, v5)) {
            0x2::table::remove<address, u64>(&mut arg1.total_user_earnings, v5)
        } else {
            0
        };
        0x2::table::add<address, u64>(&mut arg1.total_user_earnings, v5, v6 + v3);
        0x2::transfer::public_transfer<0x2::coin::Coin<T0>>(0x2::coin::from_balance<T0>(0x2::balance::split<T0>(&mut arg1.earnings_in_escrow, v3), arg5), v5);
        let v7 = RewardsClaimedEvent<T0>{
            nft_id     : v0,
            user       : v5,
            amount     : v3,
            claimed_at : v4,
        };
        0x2::event::emit<RewardsClaimedEvent<T0>>(v7);
    }
