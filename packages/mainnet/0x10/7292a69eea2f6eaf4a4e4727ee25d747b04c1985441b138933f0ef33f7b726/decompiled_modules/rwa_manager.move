module 0x8f2534a77f1f0baf8c2bc44cf1cc5ada72ffd6be515f54f8f073dc434e1a3ca2::rwa_manager {
    ...
    public entry fun admin_set_prover_pubkey(arg0: &AdminCap, arg1: &mut RWAManagerState, arg2: vector<u8>) {
        let v0 = 0x1::vector::length<u8>(&arg2);
        assert!(v0 == 32 || v0 == 0, 5);
        if (0x2::dynamic_field::exists_<vector<u8>>(&arg1.id, b"rwa_prover_pubkey_v1")) {
            0x2::dynamic_field::remove<vector<u8>, vector<u8>>(&mut arg1.id, b"rwa_prover_pubkey_v1");
        };
        if (v0 == 32) {
            0x2::dynamic_field::add<vector<u8>, vector<u8>>(&mut arg1.id, b"rwa_prover_pubkey_v1", arg2);
        };
    }
    ...
    public entry fun execute_strategy(arg0: &AgentCap, arg1: &RWAManagerState, arg2: &mut Portfolio, arg3: 0x1::string::String, arg4: vector<u8>, arg5: &0x2::clock::Clock, arg6: &mut 0x2::tx_context::TxContext) {
        ...
        let v0 = 0x2::object::id<Portfolio>(arg2);
        let v1 = 0x2::bcs::to_bytes<0x2::object::ID>(&v0);
        0x1::vector::append<u8>(&mut v1, *0x1::string::as_bytes(&arg3));
        let v2 = verify_with_prover_rwa(arg1, &arg4, &v1);
        assert!(v2, 0);
        let v3 = StrategyExecuted{
        ...
        0x2::event::emit<StrategyExecuted>(v3);
        if (v2) {
            let v4 = ZKProofVerified{
                portfolio_id : 0x2::object::id<Portfolio>(arg2),
                proof_hash   : arg4,
                verified     : true,
            };
            0x2::event::emit<ZKProofVerified>(v4);
        };
    }
    ...
    public fun has_prover_pubkey(arg0: &RWAManagerState) : bool {
        0x2::dynamic_field::exists_<vector<u8>>(&arg0.id, b"rwa_prover_pubkey_v1")
    }
    ...
    fun verify_with_prover_rwa(arg0: &RWAManagerState, arg1: &vector<u8>, arg2: &vector<u8>) : bool {
        if (0x1::vector::length<u8>(arg1) < 64) {
            if (!0x2::dynamic_field::exists_<vector<u8>>(&arg0.id, b"rwa_prover_pubkey_v1")) {
        ...
        if (0x2::dynamic_field::exists_<vector<u8>>(&arg0.id, b"rwa_prover_pubkey_v1")) {
            ...
            return 0x2::ed25519::ed25519_verify(&v0, 0x2::dynamic_field::borrow<vector<u8>, vector<u8>>(&arg0.id, b"rwa_prover_pubkey_v1"), arg2)
        };
        true
    }
