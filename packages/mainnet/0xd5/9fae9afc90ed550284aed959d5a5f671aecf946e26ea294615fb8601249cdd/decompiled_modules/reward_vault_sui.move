module 0x11b6e7ab930578ee2468e9763c68a340b05b36cbdb4813ed3c612ff7d59a1a1f::reward_vault_sui {
    ...
    public fun done_swap_v2<T0>(arg0: &mut vector<AmtGuard<T0, T0>>) {
        let v0 = 0;
        while (v0 < 0x1::vector::length<AmtGuard<T0, T0>>(arg0)) {
            let AmtGuard {
                nextCoin      : _,
                nextAmount    : _,
                finalCoinType : _,
            } = 0x1::vector::pop_back<AmtGuard<T0, T0>>(arg0);
            v0 = v0 + 1;
        };
    }

    public fun done_swap_v3<T0>(arg0: vector<AmtGuard<T0, T0>>) {
        let v0 = 0;
        while (v0 < 0x1::vector::length<AmtGuard<T0, T0>>(&arg0)) {
            let AmtGuard {
                nextCoin      : _,
                nextAmount    : _,
                finalCoinType : _,
            } = 0x1::vector::pop_back<AmtGuard<T0, T0>>(&mut arg0);
            v0 = v0 + 1;
        };
        0x1::vector::destroy_empty<AmtGuard<T0, T0>>(arg0);
    }
    ...
    public fun receive_coin<T0>(arg0: &mut RewardVault, arg1: 0x2::transfer::Receiving<0x2::coin::Coin<T0>>) {
        let v0 = 0x2::transfer::public_receive<0x2::coin::Coin<T0>>(&mut arg0.id, arg1);
        join_coin<T0>(arg0, v0);
    }
