module 0x81542f65941bc188d8b6ddb3eb3ecc78812013fff9f2350322b474578276f78::royalty {
    struct RoyaltyRule has drop {
        dummy_field: bool,
    }

    struct RoyaltyConfig has drop, store {
        royalty: u64,
    }

    public fun add<T0>(arg0: &mut 0x2::transfer_policy::TransferPolicy<T0>, arg1: &0x2::transfer_policy::TransferPolicyCap<T0>, arg2: u64) {
        assert!(arg2 <= 10000, 0);
        let v0 = RoyaltyRule{dummy_field: false};
        let v1 = RoyaltyConfig{royalty: arg2};
        0x2::transfer_policy::add_rule<T0, RoyaltyRule, RoyaltyConfig>(v0, arg0, arg1, v1);
    }

    public fun pay<T0>(arg0: &mut 0x2::transfer_policy::TransferPolicy<T0>, arg1: &mut 0x2::transfer_policy::TransferRequest<T0>, arg2: &mut 0x2::coin::Coin<0x2::sui::SUI>, arg3: &mut 0x2::tx_context::TxContext) {
        let v0 = RoyaltyRule{dummy_field: false};
        let v1 = 0x2::transfer_policy::paid<T0>(arg1) * 0x2::transfer_policy::get_rule<T0, RoyaltyRule, RoyaltyConfig>(v0, arg0).royalty / 10000;
        assert!(0x2::coin::value<0x2::sui::SUI>(arg2) >= v1, 0);
        let v2 = RoyaltyRule{dummy_field: false};
        0x2::transfer_policy::add_to_balance<T0, RoyaltyRule>(v2, arg0, 0x2::coin::split<0x2::sui::SUI>(arg2, v1, arg3));
        let v3 = RoyaltyRule{dummy_field: false};
        0x2::transfer_policy::add_receipt<T0, RoyaltyRule>(v3, arg1);
    }

    // decompiled from Move bytecode v6
}

