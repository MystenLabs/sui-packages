module 0xd60bbb359c55e2c60c2360defa75a5e8446c36cc794ac55bb14cf339f5a62bf5::vault {
    ...
    public fun drop_share(arg0: &mut UserShareRegistry, arg1: u64, arg2: u8, arg3: address) : u64 {
        let v0 = UserShareKey{
            index : arg1,
            tag   : arg2,
            user  : arg3,
        };
        0xd60bbb359c55e2c60c2360defa75a5e8446c36cc794ac55bb14cf339f5a62bf5::linked_list::drop_node<UserShareKey, u64>(&mut arg0.id, v0)
    }

    public fun force_drop_share(arg0: &mut UserShareRegistry, arg1: u64, arg2: u8, arg3: address) : u64 {
        let v0 = UserShareKey{
            index : arg1,
            tag   : arg2,
            user  : arg3,
        };
        0xd60bbb359c55e2c60c2360defa75a5e8446c36cc794ac55bb14cf339f5a62bf5::linked_list::force_drop_node<UserShareKey, u64>(&mut arg0.id, v0)
    }
    ...
    public fun withdraw_all_balance<T0, T1, T2>(arg0: &mut DepositVault<T0, T1>, arg1: &mut BidVault<T0, T2>) : (0x2::balance::Balance<T1>, 0x2::balance::Balance<T2>, vector<u64>) {
        let v0 = vector[];
        let v1 = 0x2::balance::zero<T1>();
        let v2 = 0x2::balance::withdraw_all<T1>(&mut arg0.active_sub_vault.balance);
        0x1::vector::push_back<u64>(&mut v0, 0x2::balance::value<T1>(&v2));
        0x2::balance::join<T1>(&mut v1, v2);
        let v3 = 0x2::balance::withdraw_all<T1>(&mut arg0.deactivating_sub_vault.balance);
        0x1::vector::push_back<u64>(&mut v0, 0x2::balance::value<T1>(&v3));
        0x2::balance::join<T1>(&mut v1, v3);
        let v4 = 0x2::balance::withdraw_all<T1>(&mut arg0.inactive_sub_vault.balance);
        0x1::vector::push_back<u64>(&mut v0, 0x2::balance::value<T1>(&v4));
        0x2::balance::join<T1>(&mut v1, v4);
        let v5 = 0x2::balance::withdraw_all<T1>(&mut arg0.warmup_sub_vault.balance);
        0x1::vector::push_back<u64>(&mut v0, 0x2::balance::value<T1>(&v5));
        0x2::balance::join<T1>(&mut v1, v5);
        let v6 = 0x2::balance::zero<T2>();
        let v7 = 0x2::balance::withdraw_all<T2>(&mut arg1.bidder_sub_vault.balance);
        0x1::vector::push_back<u64>(&mut v0, 0x2::balance::value<T2>(&v7));
        0x2::balance::join<T2>(&mut v6, v7);
        let v8 = 0x2::balance::withdraw_all<T2>(&mut arg1.premium_sub_vault.balance);
        0x1::vector::push_back<u64>(&mut v0, 0x2::balance::value<T2>(&v8));
        0x2::balance::join<T2>(&mut v6, v8);
        let v9 = 0x2::balance::withdraw_all<T2>(&mut arg1.performance_fee_sub_vault.balance);
        0x1::vector::push_back<u64>(&mut v0, 0x2::balance::value<T2>(&v9));
        0x2::balance::join<T2>(&mut v6, v9);
        (v1, v6, v0)
    }
