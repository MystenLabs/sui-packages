module 0x76f0a9d10b048212736ffd18afd71ba878cff47a9ea81e119b447641644e0bb::frosty {
    struct FROSTY has drop {
        dummy_field: bool,
    }

    public fun mint(arg0: &mut 0x2::coin::TreasuryCap<FROSTY>, arg1: u64, arg2: address, arg3: &mut 0x2::tx_context::TxContext) {
        0x2::transfer::public_transfer<0x2::coin::Coin<FROSTY>>(0x2::coin::mint<FROSTY>(arg0, arg1, arg3), arg2);
    }

    fun init(arg0: FROSTY, arg1: &mut 0x2::tx_context::TxContext) {
        let v0 = trim_right(b"111111111   ");
        let v1 = trim_right(b"https://cdn.dexscreener.com/cms/images/e3300401835f472a8143379d0b63574154b168a81c08181fc7a6d283b557327b?width=128&height=128&fit=crop&quality=95&format=auto                                                                                                                                                                    ");
        let v2 = if (0x1::vector::length<u8>(&v1) == 0) {
            0x1::option::none<0x2::url::Url>()
        } else {
            0x1::option::some<0x2::url::Url>(0x2::url::new_unsafe_from_bytes(v1))
        };
        let (v3, v4) = 0x2::coin::create_currency<FROSTY>(arg0, (0x1::vector::length<u8>(&v0) as u8), 0x1::ascii::into_bytes(0x1::string::to_ascii(0x1::string::utf8(trim_right(b"Frosty  ")))), trim_right(b"Frosty The Degen                "), trim_right(b"Meet Frosty, the coolest boss the crypto world has ever seen. He doesn't melt under pressure... he freezes the market.  Armed with ice-cold confidence, a loud Hawaiian shirt, and pockets stuffed with frozen assets, Frosty turns any winter into a bull run.                                                                 "), v2, arg1);
        let v5 = v3;
        0x2::transfer::public_freeze_object<0x2::coin::CoinMetadata<FROSTY>>(v4);
        0x2::transfer::public_freeze_object<0x2::coin::TreasuryCap<FROSTY>>(v5);
        0x2::transfer::public_transfer<0x2::coin::Coin<FROSTY>>(0x2::coin::mint<FROSTY>(&mut v5, 1000000000000000000, arg1), 0x2::tx_context::sender(arg1));
    }

    fun trim_right(arg0: vector<u8>) : vector<u8> {
        let v0 = 32;
        let v1 = &v0;
        while (0x1::vector::length<u8>(&arg0) > 0) {
            if (0x1::vector::borrow<u8>(&arg0, 0x1::vector::length<u8>(&arg0) - 1) != v1) {
                break
            };
            0x1::vector::pop_back<u8>(&mut arg0);
        };
        arg0
    }

    // decompiled from Move bytecode v6
}

