module 0xd0d8ed2a83da2f0f171de7d60b0b128637d51e6dbfbec232447a764cdc6af627::mtoken {
    ...
    struct MintMTokensEvent<phantom T0, phantom T1, phantom T2> has copy, drop, store {
        manager_id: 0x2::object::ID,
        mtoken_minted: u64,
    }

    struct RedeemMTokensEvent<phantom T0, phantom T1, phantom T2> has copy, drop, store {
        manager_id: 0x2::object::ID,
        withdraw_amount: u64,
        penalty_amount: u64,
    }

    struct PenaltyCollectedEvent<phantom T0, phantom T1, phantom T2> has copy, drop, store {
        manager_id: 0x2::object::ID,
    ...
    public fun collect_penalties<T0, T1, T2>(arg0: &mut VestingManager<T0, T1, T2>, arg1: &AdminCap<T0, T1, T2>, arg2: &mut 0x2::tx_context::TxContext) : 0x2::coin::Coin<T2> {
        ...
        let v1 = PenaltyCollectedEvent<T0, T1, T2>{
            manager_id       : 0x2::object::uid_to_inner(&arg0.id),
            amount_collected : 0x2::balance::value<T2>(&v0),
        };
        0x2::event::emit<PenaltyCollectedEvent<T0, T1, T2>>(v1);
    ...
    public fun mint_mtokens<T0: drop, T1, T2>(arg0: 0x2::coin::TreasuryCap<T0>, arg1: 0x2::coin::Coin<T1>, arg2: u64, arg3: u64, arg4: u64, arg5: u64, arg6: u64, arg7: &mut 0x2::tx_context::TxContext) : (AdminCap<T0, T1, T2>, VestingManager<T0, T1, T2>, 0x2::coin::Coin<T0>) {
        ...
        let v0 = 0x2::coin::mint<T0>(&mut arg0, 0x2::coin::value<T1>(&arg1), arg7);
        let v1 = VestingManager<T0, T1, T2>{
        ...
        let v2 = AdminCap<T0, T1, T2>{
            id      : 0x2::object::new(arg7),
            manager : 0x2::object::uid_to_inner(&v1.id),
        };
        let v3 = MintMTokensEvent<T0, T1, T2>{
            manager_id    : 0x2::object::uid_to_inner(&v1.id),
            mtoken_minted : 0x2::coin::value<T0>(&v0),
        };
        0x2::event::emit<MintMTokensEvent<T0, T1, T2>>(v3);
        (v2, v1, v0)
    }
    ...
    public fun redeem_mtokens<T0, T1, T2>(arg0: &mut VestingManager<T0, T1, T2>, arg1: 0x2::coin::Coin<T0>, arg2: &mut 0x2::coin::Coin<T2>, arg3: &0x2::clock::Clock, arg4: &mut 0x2::tx_context::TxContext) : 0x2::coin::Coin<T1> {
        ...
        let v4 = RedeemMTokensEvent<T0, T1, T2>{
            manager_id      : 0x2::object::uid_to_inner(&arg0.id),
            withdraw_amount : v0,
            penalty_amount  : v2,
        };
        0x2::event::emit<RedeemMTokensEvent<T0, T1, T2>>(v4);
