module 0x5cfddf8ba23be6835644a8ea22482ff6ebb0081e42cc1bc052b5f770ca8bbdea::errors {
    public fun already_locked() : u64 {
        21
    }

    public fun already_settled() : u64 {
        8
    }

    public fun bad_param() : u64 {
        33
    }

    public fun basket_bad_weights() : u64 {
        38
    }

    public fun basket_empty() : u64 {
        37
    }

    public fun basket_mode() : u64 {
        42
    }

    public fun graduated() : u64 {
        5
    }

    public fun insufficient_liquidity() : u64 {
        17
    }

    public fun invalid_fee() : u64 {
        2
    }

    public fun invalid_pit_mode() : u64 {
        10
    }

    public fun no_holders() : u64 {
        31
    }

    public fun no_yield_to_detach() : u64 {
        45
    }

    public fun not_admin() : u64 {
        18
    }

    public fun not_basket_yield() : u64 {
        39
    }

    public fun not_beneficiary() : u64 {
        22
    }

    public fun not_creator() : u64 {
        19
    }

    public fun not_graduated() : u64 {
        6
    }

    public fun not_holder_yield() : u64 {
        35
    }

    public fun not_winner() : u64 {
        9
    }

    public fun nothing_to_claim() : u64 {
        14
    }

    public fun overflow() : u64 {
        13
    }

    public fun paused() : u64 {
        1
    }

    public fun pit_not_registered() : u64 {
        26
    }

    public fun reserves_locked() : u64 {
        32
    }

    public fun retired() : u64 {
        29
    }

    public fun round_not_started() : u64 {
        15
    }

    public fun slippage() : u64 {
        4
    }

    public fun still_burnable() : u64 {
        28
    }

    public fun still_locked() : u64 {
        20
    }

    public fun too_early() : u64 {
        7
    }

    public fun unknown_rwa() : u64 {
        41
    }

    public fun unregistered_seller() : u64 {
        30
    }

    public fun unsettled_winner() : u64 {
        16
    }

    public fun use_basket_yield_collect() : u64 {
        43
    }

    public fun use_holder_yield_collect() : u64 {
        36
    }

    public fun use_instadex_collect() : u64 {
        24
    }

    public fun use_split_collect() : u64 {
        23
    }

    public fun wrong_basket() : u64 {
        40
    }

    public fun wrong_pit() : u64 {
        25
    }

    public fun wrong_pool() : u64 {
        27
    }

    public fun wrong_yield() : u64 {
        34
    }

    public fun yield_mode_conflict() : u64 {
        44
    }

    public fun zero_amount() : u64 {
        3
    }

    public fun zero_denominator() : u64 {
        11
    }

    public fun zero_in() : u64 {
        12
    }

    // decompiled from Move bytecode v7
}

