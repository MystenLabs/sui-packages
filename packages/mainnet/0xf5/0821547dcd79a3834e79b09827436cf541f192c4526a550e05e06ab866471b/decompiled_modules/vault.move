module 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::vault {
    struct WindDownStateKey has copy, drop, store {
        dummy_field: bool,
    }

    struct FinalAssetsKey has copy, drop, store {
        dummy_field: bool,
    }

    struct FinalDebtAssetsKey has copy, drop, store {
        dummy_field: bool,
    }

    struct WindDownRewardKey<phantom T0> has copy, drop, store {
        dummy_field: bool,
    }

    struct WindDownState has store {
        lifecycle: u8,
    }

    struct VaultCap has store, key {
        id: 0x2::object::UID,
        vault_id: 0x2::object::ID,
    }

    struct HarvestCap has key {
        id: 0x2::object::UID,
        vault_id: 0x2::object::ID,
        whitelisted_address: address,
    }

    struct Vault<phantom T0, phantom T1, phantom T2> has store, key {
        id: 0x2::object::UID,
        vt_treasury_cap: 0x2::coin::TreasuryCap<T2>,
        account_cap: 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::account::AccountCap,
        pool_id: 0x2::object::ID,
        target_leverage: u64,
        a_index: u8,
        b_index: u8,
        withdrawal_fees: u64,
        performance_fees: u64,
        slippage_up: u128,
        slippage_down: u128,
        deposits_enabled: bool,
        deposit_limit: u64,
        seed_vt: 0x2::balance::Balance<T2>,
        collected_withdrawal_fees: 0x2::balance::Balance<T0>,
        collected_performance_fees: 0x2::balance::Balance<T2>,
        swap_routes: 0x2::table::Table<0x1::type_name::TypeName, vector<0x360802cb106b004205cd67a706e314eaeadd8590d5f4d1cee46137566232a627::bag_value::Value>>,
    }

    public fun a_index<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) : u8 {
        arg0.a_index
    }

    public(friend) fun account_cap<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) : &0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::account::AccountCap {
        &arg0.account_cap
    }

    public(friend) fun add_swap_route<T0, T1, T2, T3>(arg0: &mut Vault<T0, T1, T2>, arg1: vector<0x360802cb106b004205cd67a706e314eaeadd8590d5f4d1cee46137566232a627::bag_value::Value>) {
        let v0 = 0x1::type_name::get<T3>();
        if (0x2::table::contains<0x1::type_name::TypeName, vector<0x360802cb106b004205cd67a706e314eaeadd8590d5f4d1cee46137566232a627::bag_value::Value>>(&arg0.swap_routes, v0)) {
            let v1 = 0x2::table::remove<0x1::type_name::TypeName, vector<0x360802cb106b004205cd67a706e314eaeadd8590d5f4d1cee46137566232a627::bag_value::Value>>(&mut arg0.swap_routes, v0);
            while (!0x1::vector::is_empty<0x360802cb106b004205cd67a706e314eaeadd8590d5f4d1cee46137566232a627::bag_value::Value>(&v1)) {
                let (_, _) = 0x360802cb106b004205cd67a706e314eaeadd8590d5f4d1cee46137566232a627::bag_value::unwrap(0x1::vector::pop_back<0x360802cb106b004205cd67a706e314eaeadd8590d5f4d1cee46137566232a627::bag_value::Value>(&mut v1));
            };
            0x1::vector::destroy_empty<0x360802cb106b004205cd67a706e314eaeadd8590d5f4d1cee46137566232a627::bag_value::Value>(v1);
        };
        0x2::table::add<0x1::type_name::TypeName, vector<0x360802cb106b004205cd67a706e314eaeadd8590d5f4d1cee46137566232a627::bag_value::Value>>(&mut arg0.swap_routes, v0, arg1);
    }

    public(friend) fun assert_active<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) {
        assert!(lifecycle<T0, T1, T2>(arg0) == 0, 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::error::vault_not_active());
    }

    public(friend) fun assert_deposits_enabled<T0, T1, T2>(arg0: &Vault<T0, T1, T2>, arg1: u64) {
        let v0 = if (arg0.deposits_enabled) {
            if (0x2::balance::value<T2>(&arg0.seed_vt) > 0) {
                arg1 < arg0.deposit_limit
            } else {
                false
            }
        } else {
            false
        };
        assert!(v0, 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::error::deposits_disabled());
    }

    public(friend) fun assert_finalized<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) {
        assert!(lifecycle<T0, T1, T2>(arg0) == 2, 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::error::vault_not_finalized());
    }

    public(friend) fun assert_harvest_cap<T0, T1, T2>(arg0: &Vault<T0, T1, T2>, arg1: &HarvestCap, arg2: &0x2::tx_context::TxContext) {
        assert!(arg1.vault_id == 0x2::object::id<Vault<T0, T1, T2>>(arg0), 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::error::invalid_harvest_cap());
        assert!(arg1.whitelisted_address == 0x2::tx_context::sender(arg2), 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::error::sender_not_whitelisted());
    }

    public(friend) fun assert_not_finalized<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) {
        assert!(lifecycle<T0, T1, T2>(arg0) != 2, 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::error::vault_not_finalized());
    }

    public(friend) fun assert_reward_collection_state<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) {
        let v0 = lifecycle<T0, T1, T2>(arg0);
        assert!(v0 == 1 || v0 == 2, 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::error::vault_not_unwinding());
    }

    public(friend) fun assert_unwinding<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) {
        assert!(lifecycle<T0, T1, T2>(arg0) == 1, 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::error::vault_not_unwinding());
    }

    public(friend) fun assert_vault_cap<T0, T1, T2>(arg0: &Vault<T0, T1, T2>, arg1: &VaultCap) {
        assert!(arg1.vault_id == 0x2::object::id<Vault<T0, T1, T2>>(arg0), 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::error::invalid_vault_cap());
    }

    public fun b_index<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) : u8 {
        arg0.b_index
    }

    public(friend) fun begin_wind_down<T0: store, T1: store, T2>(arg0: &mut Vault<T0, T1, T2>) {
        assert_active<T0, T1, T2>(arg0);
        arg0.deposits_enabled = false;
        let v0 = WindDownStateKey{dummy_field: false};
        let v1 = WindDownState{lifecycle: 1};
        0x2::dynamic_field::add<WindDownStateKey, WindDownState>(&mut arg0.id, v0, v1);
        let v2 = FinalAssetsKey{dummy_field: false};
        0x2::dynamic_field::add<FinalAssetsKey, 0x2::balance::Balance<T0>>(&mut arg0.id, v2, 0x2::balance::zero<T0>());
        let v3 = FinalDebtAssetsKey{dummy_field: false};
        0x2::dynamic_field::add<FinalDebtAssetsKey, 0x2::balance::Balance<T1>>(&mut arg0.id, v3, 0x2::balance::zero<T1>());
    }

    public(friend) fun begin_wind_down_v2<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>) {
        assert_active<T0, T1, T2>(arg0);
        arg0.deposits_enabled = false;
        let v0 = WindDownStateKey{dummy_field: false};
        let v1 = WindDownState{lifecycle: 1};
        0x2::dynamic_field::add<WindDownStateKey, WindDownState>(&mut arg0.id, v0, v1);
        let v2 = FinalAssetsKey{dummy_field: false};
        0x2::dynamic_field::add<FinalAssetsKey, 0x2::balance::Balance<T0>>(&mut arg0.id, v2, 0x2::balance::zero<T0>());
        let v3 = FinalDebtAssetsKey{dummy_field: false};
        0x2::dynamic_field::add<FinalDebtAssetsKey, 0x2::balance::Balance<T1>>(&mut arg0.id, v3, 0x2::balance::zero<T1>());
    }

    public(friend) fun burn_vt<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: 0x2::coin::Coin<T2>) {
        0x2::coin::burn<T2>(&mut arg0.vt_treasury_cap, arg1);
    }

    public(friend) fun close_position_<T0, T1, T2>(arg0: &Vault<T0, T1, T2>, arg1: 0x2::coin::Coin<T1>, arg2: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::storage::Storage, arg3: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::pool::Pool<T0>, arg4: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::pool::Pool<T1>, arg5: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v2::Incentive, arg6: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v3::Incentive, arg7: &0xca441b44943c16be0e6e23c5a955bb971537ea3289ae8016fbf33fffe1fd210f::oracle::PriceOracle, arg8: &0x2::clock::Clock, arg9: &mut 0x2::tx_context::TxContext) : (0x2::coin::Coin<T0>, 0x2::balance::Balance<T1>) {
        let v0 = 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v3::repay_with_account_cap<T1>(arg8, arg7, arg2, arg4, arg0.b_index, arg1, arg5, arg6, &arg0.account_cap);
        let (v1, v2) = info<T0, T1, T2>(arg0, arg3, arg4, arg2);
        assert!(v2 == 0, 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::error::position_not_closed());
        let v3 = 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v3::withdraw_with_account_cap<T0>(arg8, arg7, arg2, arg3, arg0.a_index, v1, arg5, arg6, &arg0.account_cap);
        let (v4, v5) = info<T0, T1, T2>(arg0, arg3, arg4, arg2);
        assert!(v4 == 0 && v5 == 0, 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::error::position_not_closed());
        (0x2::coin::from_balance<T0>(v3, arg9), v0)
    }

    public(friend) fun collect_fees<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: 0x2::balance::Balance<T0>) {
        0x2::balance::join<T0>(&mut arg0.collected_withdrawal_fees, arg1);
    }

    public(friend) fun collect_performance_fees<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: 0x2::balance::Balance<T2>) {
        0x2::balance::join<T2>(&mut arg0.collected_performance_fees, arg1);
    }

    public(friend) fun deposit<T0, T1, T2>(arg0: &Vault<T0, T1, T2>, arg1: 0x2::coin::Coin<T0>, arg2: u64, arg3: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::storage::Storage, arg4: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::pool::Pool<T0>, arg5: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::pool::Pool<T1>, arg6: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive::Incentive, arg7: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v2::Incentive, arg8: &0xca441b44943c16be0e6e23c5a955bb971537ea3289ae8016fbf33fffe1fd210f::oracle::PriceOracle, arg9: &0x2::clock::Clock) : 0x2::balance::Balance<T1> {
        0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v2::deposit_with_account_cap<T0>(arg9, arg3, arg4, arg0.a_index, arg1, arg6, arg7, &arg0.account_cap);
        0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v2::borrow_with_account_cap<T1>(arg9, arg8, arg3, arg5, arg0.b_index, arg2, arg7, &arg0.account_cap)
    }

    public(friend) fun deposit_<T0, T1, T2>(arg0: &Vault<T0, T1, T2>, arg1: 0x2::coin::Coin<T0>, arg2: u64, arg3: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::storage::Storage, arg4: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::pool::Pool<T0>, arg5: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::pool::Pool<T1>, arg6: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v2::Incentive, arg7: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v3::Incentive, arg8: &0xca441b44943c16be0e6e23c5a955bb971537ea3289ae8016fbf33fffe1fd210f::oracle::PriceOracle, arg9: &0x2::clock::Clock, arg10: &mut 0x2::tx_context::TxContext) : 0x2::balance::Balance<T1> {
        let v0 = get_adjusted_borrow_amount(arg2, get_borrow_fees_df<T0, T1, T2>(arg0));
        0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v3::deposit_with_account_cap<T0>(arg9, arg3, arg4, arg0.a_index, arg1, arg6, arg7, account_cap<T0, T1, T2>(arg0));
        let v1 = 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v3::borrow_with_account_cap<T1>(arg9, arg8, arg3, arg5, arg0.b_index, v0 + v0 / 10000, arg6, arg7, &arg0.account_cap);
        if (0x2::balance::value<T1>(&v1) > arg2) {
            0x2::balance::destroy_zero<T1>(0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v3::repay_with_account_cap<T1>(arg9, arg8, arg3, arg5, arg0.b_index, 0x2::coin::from_balance<T1>(0x2::balance::split<T1>(&mut v1, 0x2::balance::value<T1>(&v1) - arg2), arg10), arg6, arg7, &arg0.account_cap));
        };
        v1
    }

    public fun fee_scaling() : u64 {
        1000000
    }

    public fun final_assets<T0: store, T1, T2>(arg0: &Vault<T0, T1, T2>) : u64 {
        let v0 = FinalAssetsKey{dummy_field: false};
        if (!0x2::dynamic_field::exists_<FinalAssetsKey>(&arg0.id, v0)) {
            return 0
        };
        let v1 = FinalAssetsKey{dummy_field: false};
        0x2::balance::value<T0>(0x2::dynamic_field::borrow<FinalAssetsKey, 0x2::balance::Balance<T0>>(&arg0.id, v1))
    }

    public fun final_assets_v2<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) : u64 {
        let v0 = FinalAssetsKey{dummy_field: false};
        if (!0x2::dynamic_field::exists_<FinalAssetsKey>(&arg0.id, v0)) {
            return 0
        };
        let v1 = FinalAssetsKey{dummy_field: false};
        0x2::balance::value<T0>(0x2::dynamic_field::borrow<FinalAssetsKey, 0x2::balance::Balance<T0>>(&arg0.id, v1))
    }

    public fun final_debt_assets<T0, T1: store, T2>(arg0: &Vault<T0, T1, T2>) : u64 {
        let v0 = FinalDebtAssetsKey{dummy_field: false};
        if (!0x2::dynamic_field::exists_<FinalDebtAssetsKey>(&arg0.id, v0)) {
            return 0
        };
        let v1 = FinalDebtAssetsKey{dummy_field: false};
        0x2::balance::value<T1>(0x2::dynamic_field::borrow<FinalDebtAssetsKey, 0x2::balance::Balance<T1>>(&arg0.id, v1))
    }

    public fun final_debt_assets_v2<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) : u64 {
        let v0 = FinalDebtAssetsKey{dummy_field: false};
        if (!0x2::dynamic_field::exists_<FinalDebtAssetsKey>(&arg0.id, v0)) {
            return 0
        };
        let v1 = FinalDebtAssetsKey{dummy_field: false};
        0x2::balance::value<T1>(0x2::dynamic_field::borrow<FinalDebtAssetsKey, 0x2::balance::Balance<T1>>(&arg0.id, v1))
    }

    public(friend) fun finalize_wind_down<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>) {
        assert_unwinding<T0, T1, T2>(arg0);
        assert!(0x2::balance::value<T0>(&arg0.collected_withdrawal_fees) == 0 && 0x2::balance::value<T2>(&arg0.collected_performance_fees) == 0, 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::error::unsettled_fees());
        let v0 = WindDownStateKey{dummy_field: false};
        0x2::dynamic_field::borrow_mut<WindDownStateKey, WindDownState>(&mut arg0.id, v0).lifecycle = 2;
    }

    fun get_adjusted_borrow_amount(arg0: u64, arg1: u64) : u64 {
        let v0 = 10000;
        (((arg0 as u256) * (v0 as u256) / ((v0 as u256) - (arg1 as u256))) as u64) + 1
    }

    public(friend) fun get_borrow_fees_df<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) : u64 {
        *0x2::dynamic_field::borrow<vector<u8>, u64>(&arg0.id, b"borrow_fees")
    }

    public fun get_swap_route<T0, T1, T2, T3>(arg0: &Vault<T0, T1, T2>) : vector<0x360802cb106b004205cd67a706e314eaeadd8590d5f4d1cee46137566232a627::bag_value::Value> {
        *0x2::table::borrow<0x1::type_name::TypeName, vector<0x360802cb106b004205cd67a706e314eaeadd8590d5f4d1cee46137566232a627::bag_value::Value>>(&arg0.swap_routes, 0x1::type_name::get<T3>())
    }

    public fun harvest_cap_vault_id(arg0: &HarvestCap) : 0x2::object::ID {
        arg0.vault_id
    }

    public fun info<T0, T1, T2>(arg0: &Vault<T0, T1, T2>, arg1: &0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::pool::Pool<T0>, arg2: &0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::pool::Pool<T1>, arg3: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::storage::Storage) : (u64, u64) {
        (0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::pool::unnormal_amount<T0>(arg1, (0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::logic::user_collateral_balance(arg3, arg0.a_index, 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::account::account_owner(&arg0.account_cap)) as u64)), 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::pool::unnormal_amount<T1>(arg2, (0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::logic::user_loan_balance(arg3, arg0.b_index, 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::account::account_owner(&arg0.account_cap)) as u64)))
    }

    public(friend) fun initialize<T0, T1, T2>(arg0: 0x2::coin::TreasuryCap<T2>, arg1: 0x2::object::ID, arg2: u64, arg3: u8, arg4: u8, arg5: u128, arg6: u128, arg7: u64, arg8: u64, arg9: u64, arg10: &mut 0x2::tx_context::TxContext) : 0x2::object::ID {
        let v0 = Vault<T0, T1, T2>{
            id                         : 0x2::object::new(arg10),
            vt_treasury_cap            : arg0,
            account_cap                : 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::lending::create_account(arg10),
            pool_id                    : arg1,
            target_leverage            : arg2,
            a_index                    : arg3,
            b_index                    : arg4,
            withdrawal_fees            : arg7,
            performance_fees           : arg8,
            slippage_up                : arg5,
            slippage_down              : arg6,
            deposits_enabled           : false,
            deposit_limit              : arg9,
            seed_vt                    : 0x2::balance::zero<T2>(),
            collected_withdrawal_fees  : 0x2::balance::zero<T0>(),
            collected_performance_fees : 0x2::balance::zero<T2>(),
            swap_routes                : 0x2::table::new<0x1::type_name::TypeName, vector<0x360802cb106b004205cd67a706e314eaeadd8590d5f4d1cee46137566232a627::bag_value::Value>>(arg10),
        };
        let v1 = 0x2::object::id<Vault<T0, T1, T2>>(&v0);
        let v2 = VaultCap{
            id       : 0x2::object::new(arg10),
            vault_id : v1,
        };
        let v3 = HarvestCap{
            id                  : 0x2::object::new(arg10),
            vault_id            : v1,
            whitelisted_address : 0x2::tx_context::sender(arg10),
        };
        0x2::transfer::share_object<Vault<T0, T1, T2>>(v0);
        0x2::transfer::share_object<HarvestCap>(v3);
        0x2::transfer::transfer<VaultCap>(v2, 0x2::tx_context::sender(arg10));
        v1
    }

    public(friend) fun join_final_assets<T0: store, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: 0x2::balance::Balance<T0>) {
        assert_unwinding<T0, T1, T2>(arg0);
        let v0 = FinalAssetsKey{dummy_field: false};
        0x2::balance::join<T0>(0x2::dynamic_field::borrow_mut<FinalAssetsKey, 0x2::balance::Balance<T0>>(&mut arg0.id, v0), arg1);
    }

    public(friend) fun join_final_assets_v2<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: 0x2::balance::Balance<T0>) {
        assert_unwinding<T0, T1, T2>(arg0);
        let v0 = FinalAssetsKey{dummy_field: false};
        0x2::balance::join<T0>(0x2::dynamic_field::borrow_mut<FinalAssetsKey, 0x2::balance::Balance<T0>>(&mut arg0.id, v0), arg1);
    }

    public(friend) fun join_final_debt_assets<T0, T1: store, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: 0x2::balance::Balance<T1>) {
        assert_unwinding<T0, T1, T2>(arg0);
        let v0 = FinalDebtAssetsKey{dummy_field: false};
        0x2::balance::join<T1>(0x2::dynamic_field::borrow_mut<FinalDebtAssetsKey, 0x2::balance::Balance<T1>>(&mut arg0.id, v0), arg1);
    }

    public(friend) fun join_final_debt_assets_v2<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: 0x2::balance::Balance<T1>) {
        assert_unwinding<T0, T1, T2>(arg0);
        let v0 = FinalDebtAssetsKey{dummy_field: false};
        0x2::balance::join<T1>(0x2::dynamic_field::borrow_mut<FinalDebtAssetsKey, 0x2::balance::Balance<T1>>(&mut arg0.id, v0), arg1);
    }

    public(friend) fun join_settled_reward_a<T0: store, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: 0x2::balance::Balance<T0>) {
        assert_finalized<T0, T1, T2>(arg0);
        let v0 = FinalAssetsKey{dummy_field: false};
        0x2::balance::join<T0>(0x2::dynamic_field::borrow_mut<FinalAssetsKey, 0x2::balance::Balance<T0>>(&mut arg0.id, v0), arg1);
    }

    public(friend) fun join_settled_reward_a_v2<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: 0x2::balance::Balance<T0>) {
        assert_finalized<T0, T1, T2>(arg0);
        let v0 = FinalAssetsKey{dummy_field: false};
        0x2::balance::join<T0>(0x2::dynamic_field::borrow_mut<FinalAssetsKey, 0x2::balance::Balance<T0>>(&mut arg0.id, v0), arg1);
    }

    public(friend) fun join_settled_reward_b<T0, T1: store, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: 0x2::balance::Balance<T1>) {
        assert_finalized<T0, T1, T2>(arg0);
        let v0 = FinalDebtAssetsKey{dummy_field: false};
        0x2::balance::join<T1>(0x2::dynamic_field::borrow_mut<FinalDebtAssetsKey, 0x2::balance::Balance<T1>>(&mut arg0.id, v0), arg1);
    }

    public(friend) fun join_settled_reward_b_v2<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: 0x2::balance::Balance<T1>) {
        assert_finalized<T0, T1, T2>(arg0);
        let v0 = FinalDebtAssetsKey{dummy_field: false};
        0x2::balance::join<T1>(0x2::dynamic_field::borrow_mut<FinalDebtAssetsKey, 0x2::balance::Balance<T1>>(&mut arg0.id, v0), arg1);
    }

    public fun leverage<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) : u64 {
        arg0.target_leverage
    }

    public fun leverage_scaling() : u64 {
        1000000
    }

    public fun lifecycle<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) : u8 {
        let v0 = WindDownStateKey{dummy_field: false};
        if (!0x2::dynamic_field::exists_<WindDownStateKey>(&arg0.id, v0)) {
            return 0
        };
        let v1 = WindDownStateKey{dummy_field: false};
        0x2::dynamic_field::borrow<WindDownStateKey, WindDownState>(&arg0.id, v1).lifecycle
    }

    public(friend) fun mint_vt<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: u64, arg2: &mut 0x2::tx_context::TxContext) : 0x2::coin::Coin<T2> {
        0x2::coin::mint<T2>(&mut arg0.vt_treasury_cap, arg1, arg2)
    }

    public fun performance_fees<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) : (u64, u64) {
        (arg0.performance_fees, 1000000)
    }

    public fun pool_id<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) : 0x2::object::ID {
        arg0.pool_id
    }

    public(friend) fun seed<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: 0x2::balance::Balance<T2>) {
        0x2::balance::join<T2>(&mut arg0.seed_vt, arg1);
    }

    public(friend) fun set_borrow_fees_df<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: u64) {
        0x2::dynamic_field::remove_if_exists<vector<u8>, u64>(&mut arg0.id, b"borrow_fees");
        0x2::dynamic_field::add<vector<u8>, u64>(&mut arg0.id, b"borrow_fees", arg1);
    }

    public(friend) fun set_deposit_limit<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: u64) {
        arg0.deposit_limit = arg1;
    }

    public(friend) fun set_deposits_enabled<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: bool) {
        arg0.deposits_enabled = arg1;
    }

    public(friend) fun set_harvest_whitelisted_address(arg0: &mut HarvestCap, arg1: address) {
        arg0.whitelisted_address = arg1;
    }

    public(friend) fun set_slippage<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: u128, arg2: u128) {
        arg0.slippage_up = arg1;
        arg0.slippage_down = arg2;
    }

    public fun slippage<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) : (u128, u128) {
        (arg0.slippage_up, arg0.slippage_down)
    }

    public(friend) fun store_wind_down_reward<T0, T1, T2, T3>(arg0: &mut Vault<T0, T1, T2>, arg1: 0x2::balance::Balance<T3>) {
        assert_reward_collection_state<T0, T1, T2>(arg0);
        assert!(0x2::balance::value<T3>(&arg1) > 0, 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::error::reward_balance_zero());
        let v0 = WindDownRewardKey<T3>{dummy_field: false};
        if (0x2::dynamic_field::exists_with_type<WindDownRewardKey<T3>, 0x2::balance::Balance<T3>>(&arg0.id, v0)) {
            0x2::balance::join<T3>(0x2::dynamic_field::borrow_mut<WindDownRewardKey<T3>, 0x2::balance::Balance<T3>>(&mut arg0.id, v0), arg1);
        } else {
            0x2::dynamic_field::add<WindDownRewardKey<T3>, 0x2::balance::Balance<T3>>(&mut arg0.id, v0, arg1);
        };
    }

    public fun stored_wind_down_reward<T0, T1, T2, T3>(arg0: &Vault<T0, T1, T2>) : u64 {
        let v0 = WindDownRewardKey<T3>{dummy_field: false};
        if (0x2::dynamic_field::exists_with_type<WindDownRewardKey<T3>, 0x2::balance::Balance<T3>>(&arg0.id, v0)) {
            0x2::balance::value<T3>(0x2::dynamic_field::borrow<WindDownRewardKey<T3>, 0x2::balance::Balance<T3>>(&arg0.id, v0))
        } else {
            0
        }
    }

    public(friend) fun take_final_assets<T0: store, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: u64) : 0x2::balance::Balance<T0> {
        assert_finalized<T0, T1, T2>(arg0);
        let v0 = FinalAssetsKey{dummy_field: false};
        0x2::balance::split<T0>(0x2::dynamic_field::borrow_mut<FinalAssetsKey, 0x2::balance::Balance<T0>>(&mut arg0.id, v0), arg1)
    }

    public(friend) fun take_final_assets_v2<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: u64) : 0x2::balance::Balance<T0> {
        assert_finalized<T0, T1, T2>(arg0);
        let v0 = FinalAssetsKey{dummy_field: false};
        0x2::balance::split<T0>(0x2::dynamic_field::borrow_mut<FinalAssetsKey, 0x2::balance::Balance<T0>>(&mut arg0.id, v0), arg1)
    }

    public(friend) fun take_final_debt_assets<T0, T1: store, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: u64) : 0x2::balance::Balance<T1> {
        assert_finalized<T0, T1, T2>(arg0);
        let v0 = FinalDebtAssetsKey{dummy_field: false};
        0x2::balance::split<T1>(0x2::dynamic_field::borrow_mut<FinalDebtAssetsKey, 0x2::balance::Balance<T1>>(&mut arg0.id, v0), arg1)
    }

    public(friend) fun take_final_debt_assets_v2<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: u64) : 0x2::balance::Balance<T1> {
        assert_finalized<T0, T1, T2>(arg0);
        let v0 = FinalDebtAssetsKey{dummy_field: false};
        0x2::balance::split<T1>(0x2::dynamic_field::borrow_mut<FinalDebtAssetsKey, 0x2::balance::Balance<T1>>(&mut arg0.id, v0), arg1)
    }

    public(friend) fun take_wind_down_reward<T0, T1, T2, T3>(arg0: &mut Vault<T0, T1, T2>, arg1: &HarvestCap, arg2: &0x2::tx_context::TxContext) : 0x2::balance::Balance<T3> {
        assert_finalized<T0, T1, T2>(arg0);
        assert_harvest_cap<T0, T1, T2>(arg0, arg1, arg2);
        let v0 = WindDownRewardKey<T3>{dummy_field: false};
        assert!(0x2::dynamic_field::exists_with_type<WindDownRewardKey<T3>, 0x2::balance::Balance<T3>>(&arg0.id, v0), 0x81d517bd4f52196dad796208491daf0b6614080759acb36a612a354a19d2bcdc::error::reward_not_stored());
        0x2::dynamic_field::remove<WindDownRewardKey<T3>, 0x2::balance::Balance<T3>>(&mut arg0.id, v0)
    }

    public fun total_vt_supply<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) : u64 {
        0x2::coin::total_supply<T2>(&arg0.vt_treasury_cap)
    }

    public(friend) fun update_leverage<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: u64) {
        arg0.target_leverage = arg1;
    }

    public(friend) fun update_performance_fees<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: u64) {
        arg0.performance_fees = arg1;
    }

    public(friend) fun update_withdrawal_fees<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: u64) {
        arg0.withdrawal_fees = arg1;
    }

    public(friend) fun withdraw<T0, T1, T2>(arg0: &Vault<T0, T1, T2>, arg1: 0x2::coin::Coin<T1>, arg2: u64, arg3: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::storage::Storage, arg4: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::pool::Pool<T0>, arg5: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::pool::Pool<T1>, arg6: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive::Incentive, arg7: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v2::Incentive, arg8: &0xca441b44943c16be0e6e23c5a955bb971537ea3289ae8016fbf33fffe1fd210f::oracle::PriceOracle, arg9: &0x2::clock::Clock, arg10: &mut 0x2::tx_context::TxContext) : 0x2::coin::Coin<T0> {
        0x2::balance::destroy_zero<T1>(0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v2::repay_with_account_cap<T1>(arg9, arg8, arg3, arg5, arg0.b_index, arg1, arg7, &arg0.account_cap));
        0x2::coin::from_balance<T0>(0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v2::withdraw_with_account_cap<T0>(arg9, arg8, arg3, arg4, arg0.a_index, arg2, arg6, arg7, &arg0.account_cap), arg10)
    }

    public(friend) fun withdraw_<T0, T1, T2>(arg0: &Vault<T0, T1, T2>, arg1: 0x2::coin::Coin<T1>, arg2: u64, arg3: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::storage::Storage, arg4: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::pool::Pool<T0>, arg5: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::pool::Pool<T1>, arg6: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v2::Incentive, arg7: &mut 0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v3::Incentive, arg8: &0xca441b44943c16be0e6e23c5a955bb971537ea3289ae8016fbf33fffe1fd210f::oracle::PriceOracle, arg9: &0x2::clock::Clock, arg10: &mut 0x2::tx_context::TxContext) : 0x2::coin::Coin<T0> {
        0x2::balance::destroy_zero<T1>(0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v3::repay_with_account_cap<T1>(arg9, arg8, arg3, arg5, arg0.b_index, arg1, arg6, arg7, &arg0.account_cap));
        0x2::coin::from_balance<T0>(0xd899cf7d2b5db716bd2cf55599fb0d5ee38a3061e7b6bb6eebf73fa5bc4c81ca::incentive_v3::withdraw_with_account_cap<T0>(arg9, arg8, arg3, arg4, arg0.a_index, arg2, arg6, arg7, &arg0.account_cap), arg10)
    }

    public(friend) fun withdraw_performance_fees<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: u64) : 0x2::balance::Balance<T2> {
        0x2::balance::split<T2>(&mut arg0.collected_performance_fees, arg1)
    }

    public(friend) fun withdraw_withdrawal_fees<T0, T1, T2>(arg0: &mut Vault<T0, T1, T2>, arg1: u64) : 0x2::balance::Balance<T0> {
        0x2::balance::split<T0>(&mut arg0.collected_withdrawal_fees, arg1)
    }

    public fun withdrawal_fees<T0, T1, T2>(arg0: &Vault<T0, T1, T2>) : (u64, u64) {
        (arg0.withdrawal_fees, 1000000)
    }

    // decompiled from Move bytecode v7
}

