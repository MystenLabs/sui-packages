module 0x5f51d02fa049194239ffeac3e446a0020e7bbfc5d9149ff888366c24b2456b1::SRMV1 {
    ...
    public entry fun emergency_withdraw<T0, T1>(arg0: &mut Pool<T0, T1>, arg1: &Config, arg2: &0x2::clock::Clock, arg3: &mut 0x2::tx_context::TxContext) {
        ...
        let v3 = 0x2::balance::value<T0>(&arg0.swap_balance_a);
        let v4 = 0x2::balance::value<T0>(&arg0.burn_balance_a);
        let v5 = 0x2::balance::value<T1>(&arg0.burn_balance_b);
        let v6 = 0x2::balance::value<T0>(&arg0.creator_balance_a);
        let v7 = 0x2::balance::value<T0>(&arg0.reward_balance_a);
        let v8 = 0x2::balance::zero<T0>();
        0x2::balance::join<T0>(&mut v8, 0x2::balance::split<T0>(&mut arg0.balance_a, v1));
        0x2::balance::join<T0>(&mut v8, 0x2::balance::split<T0>(&mut arg0.swap_balance_a, v3));
        0x2::balance::join<T0>(&mut v8, 0x2::balance::split<T0>(&mut arg0.burn_balance_a, v4));
        0x2::balance::join<T0>(&mut v8, 0x2::balance::split<T0>(&mut arg0.creator_balance_a, v6));
        0x2::balance::join<T0>(&mut v8, 0x2::balance::split<T0>(&mut arg0.reward_balance_a, v7));
        let v9 = 0x2::balance::zero<T1>();
        0x2::balance::join<T1>(&mut v9, 0x2::balance::split<T1>(&mut arg0.balance_b, v2));
        0x2::balance::join<T1>(&mut v9, 0x2::balance::split<T1>(&mut arg0.burn_balance_b, v5));
        destroy_zero_or_transfer_balance<T0>(v8, v0, arg3);
        destroy_zero_or_transfer_balance<T1>(v9, v0, arg3);
        let v10 = EmergencyWithdraw{
            pool_id     : 0x2::object::id<Pool<T0, T1>>(arg0),
            total_a     : 0x2::balance::value<T0>(&v8),
            total_b     : 0x2::balance::value<T1>(&v9),
            reserve_a   : v1,
            reserve_b   : v2,
            swap_fees_a : v3,
            burn_a      : v4,
        ...
        0x2::event::emit<EmergencyWithdraw>(v10);
    }
