module 0x43e99dc83d4c41b8db7ae4610f232d59389dc70ec9faadbcd810fc284b4bd561::point {
    struct PointKey<phantom T0> has store {
        dummy_field: bool,
    }

    struct AddPointRequest<phantom T0> has key {
        id: 0x2::object::UID,
        owner: address,
        action: 0x1::ascii::String,
        value: u256,
    }

    struct SubPointRequest<phantom T0> has key {
        id: 0x2::object::UID,
        owner: address,
        action: 0x1::ascii::String,
        value: u256,
    }

    struct StakePointRequest<phantom T0> has key {
        id: 0x2::object::UID,
        owner: address,
        action: 0x1::ascii::String,
        weight: u256,
        duration: u64,
        timestamp: u64,
    }

    struct UnstakePointRequest<phantom T0> has key {
        id: 0x2::object::UID,
        owner: address,
        action: 0x1::ascii::String,
        weight: u256,
        duration: u64,
        timestamp: u64,
    }

    struct BorrowPointRequest<phantom T0> has key {
        id: 0x2::object::UID,
        owner: address,
        collateral_amount: u256,
        borrow_amount: u256,
        duration: u64,
        timestamp: u64,
    }

    struct RepayPointRequest<phantom T0> has key {
        id: 0x2::object::UID,
        owner: address,
        repay_amount: u256,
        timestamp: u64,
    }

    struct UserInfo has copy, drop, store {
        points: 0x2::vec_map::VecMap<0x1::ascii::String, u256>,
        configs: 0x2::vec_map::VecMap<0x1::ascii::String, Config>,
    }

    struct Config has copy, drop, store {
        weight: u256,
        last_update: u64,
        duration: u64,
    }

    struct PointDashBoard<phantom T0> has store, key {
        id: 0x2::object::UID,
        total_points: u256,
        user_infos: 0x2::table::Table<address, UserInfo>,
    }

    struct LiquidlinkAddPointEvent<phantom T0> has copy, drop {
        owner: address,
        action: 0x1::ascii::String,
        req: 0x2::object::ID,
        value: u256,
    }

    struct LiquidlinkSubPointEvent<phantom T0> has copy, drop {
        owner: address,
        action: 0x1::ascii::String,
        req: 0x2::object::ID,
        value: u256,
    }

    struct LiquidlinkStakePointEvent<phantom T0> has copy, drop {
        owner: address,
        action: 0x1::ascii::String,
        req: 0x2::object::ID,
        weight: u256,
        timestamp: u64,
        duration: u64,
    }

    struct LiquidlinkUnstakePointEvent<phantom T0> has copy, drop {
        owner: address,
        action: 0x1::ascii::String,
        req: 0x2::object::ID,
        weight: u256,
        timestamp: u64,
        duration: u64,
    }

    public(friend) fun add_point<T0: drop>(arg0: &mut PointDashBoard<T0>, arg1: AddPointRequest<T0>) {
        let AddPointRequest {
            id     : v0,
            owner  : v1,
            action : v2,
            value  : v3,
        } = arg1;
        0x2::object::delete(v0);
        add_point_internal<T0>(arg0, v1, v2, v3);
    }

    public(friend) fun add_point_internal<T0: drop>(arg0: &mut PointDashBoard<T0>, arg1: address, arg2: 0x1::ascii::String, arg3: u256) {
        init_user_info<T0>(arg0, arg1);
        let v0 = 0x2::table::borrow_mut<address, UserInfo>(&mut arg0.user_infos, arg1);
        if (!0x2::vec_map::contains<0x1::ascii::String, u256>(&v0.points, &arg2)) {
            0x2::vec_map::insert<0x1::ascii::String, u256>(&mut v0.points, arg2, arg3);
        } else {
            *0x2::vec_map::get_mut<0x1::ascii::String, u256>(&mut v0.points, &arg2) = *0x2::vec_map::get<0x1::ascii::String, u256>(&v0.points, &arg2) + arg3;
        };
        arg0.total_points = arg0.total_points + arg3;
    }

    public fun borrow_action<T0>(arg0: &AddPointRequest<T0>) : 0x1::ascii::String {
        arg0.action
    }

    public fun borrow_amount<T0>(arg0: &AddPointRequest<T0>) : u256 {
        arg0.value
    }

    public fun borrow_owner<T0>(arg0: &AddPointRequest<T0>) : address {
        arg0.owner
    }

    fun calculate_config_points(arg0: &Config, arg1: u64) : u256 {
        if (arg0.duration == 0) {
            return 0
        };
        (arg0.weight as u256) * ((arg1 - arg0.last_update) as u256) / (arg0.duration as u256)
    }

    fun checkpoint(arg0: &mut Config, arg1: u64, arg2: u256, arg3: u64) : u256 {
        if (arg1 >= arg0.last_update) {
            arg0.weight = arg2;
            arg0.last_update = arg1;
            arg0.duration = arg3;
            calculate_config_points(arg0, arg1)
        } else {
            0
        }
    }

    public(friend) fun drop_point_key<T0>(arg0: PointKey<T0>) {
        let PointKey {  } = arg0;
    }

    public fun get_user_info<T0>(arg0: &PointDashBoard<T0>, arg1: address) : 0x1::option::Option<UserInfo> {
        if (0x2::table::contains<address, UserInfo>(&arg0.user_infos, arg1)) {
            0x1::option::some<UserInfo>(*0x2::table::borrow<address, UserInfo>(&arg0.user_infos, arg1))
        } else {
            0x1::option::none<UserInfo>()
        }
    }

    public fun get_user_info_point_by_action<T0>(arg0: &PointDashBoard<T0>, arg1: address, arg2: 0x1::ascii::String, arg3: &0x2::clock::Clock) : u256 {
        if (!0x2::table::contains<address, UserInfo>(&arg0.user_infos, arg1)) {
            0
        } else {
            let v1 = *0x2::table::borrow<address, UserInfo>(&arg0.user_infos, arg1);
            let v2 = if (0x2::vec_map::contains<0x1::ascii::String, u256>(&v1.points, &arg2)) {
                *0x2::vec_map::get<0x1::ascii::String, u256>(&v1.points, &arg2)
            } else {
                0
            };
            let v3 = if (0x2::vec_map::contains<0x1::ascii::String, Config>(&v1.configs, &arg2)) {
                calculate_config_points(0x2::vec_map::get<0x1::ascii::String, Config>(&v1.configs, &arg2), 0x2::clock::timestamp_ms(arg3))
            } else {
                0
            };
            v2 + v3
        }
    }

    public fun get_user_info_points<T0>(arg0: &PointDashBoard<T0>, arg1: address, arg2: &0x2::clock::Clock) : u256 {
        let v0 = 0;
        let v1 = get_user_info<T0>(arg0, arg1);
        if (0x1::option::is_some<UserInfo>(&v1)) {
            let v2 = 0x1::option::borrow<UserInfo>(&v1);
            let v3 = 0x2::vec_map::keys<0x1::ascii::String, u256>(&v2.points);
            let v4 = 0;
            while (v4 < 0x1::vector::length<0x1::ascii::String>(&v3)) {
                v0 = v0 + *0x2::vec_map::get<0x1::ascii::String, u256>(&v2.points, 0x1::vector::borrow<0x1::ascii::String>(&v3, v4));
                v4 = v4 + 1;
            };
            let v5 = 0x2::vec_map::keys<0x1::ascii::String, Config>(&v2.configs);
            let v6 = 0;
            while (v6 < 0x1::vector::length<0x1::ascii::String>(&v5)) {
                v0 = v0 + calculate_config_points(0x2::vec_map::get<0x1::ascii::String, Config>(&v2.configs, 0x1::vector::borrow<0x1::ascii::String>(&v5, v6)), 0x2::clock::timestamp_ms(arg2));
                v6 = v6 + 1;
            };
        };
        v0
    }

    fun init_user_info<T0: drop>(arg0: &mut PointDashBoard<T0>, arg1: address) {
        if (!0x2::table::contains<address, UserInfo>(&arg0.user_infos, arg1)) {
            let v0 = UserInfo{
                points  : 0x2::vec_map::empty<0x1::ascii::String, u256>(),
                configs : 0x2::vec_map::empty<0x1::ascii::String, Config>(),
            };
            0x2::table::add<address, UserInfo>(&mut arg0.user_infos, arg1, v0);
        };
    }

    public(friend) fun new_point_dashboard<T0>(arg0: &mut 0x2::tx_context::TxContext) : PointDashBoard<T0> {
        PointDashBoard<T0>{
            id           : 0x2::object::new(arg0),
            total_points : 0,
            user_infos   : 0x2::table::new<address, UserInfo>(arg0),
        }
    }

    public(friend) fun new_point_key<T0>() : PointKey<T0> {
        PointKey<T0>{dummy_field: false}
    }

    public fun send_add_point_req<T0: drop>(arg0: &T0, arg1: 0x1::ascii::String, arg2: u256, arg3: &mut 0x2::tx_context::TxContext) {
        let v0 = 0x2::tx_context::sender(arg3);
        send_add_point_req_<T0>(0x43e99dc83d4c41b8db7ae4610f232d59389dc70ec9faadbcd810fc284b4bd561::constant::point_updater(), v0, arg1, arg2, arg3);
    }

    fun send_add_point_req_<T0: drop>(arg0: address, arg1: address, arg2: 0x1::ascii::String, arg3: u256, arg4: &mut 0x2::tx_context::TxContext) {
        let v0 = AddPointRequest<T0>{
            id     : 0x2::object::new(arg4),
            owner  : arg1,
            action : arg2,
            value  : arg3,
        };
        let v1 = LiquidlinkAddPointEvent<T0>{
            owner  : arg1,
            action : arg2,
            req    : 0x2::object::id<AddPointRequest<T0>>(&v0),
            value  : arg3,
        };
        0x2::event::emit<LiquidlinkAddPointEvent<T0>>(v1);
        0x2::transfer::transfer<AddPointRequest<T0>>(v0, arg0);
    }

    public fun send_add_point_req_with_owner<T0: drop>(arg0: &T0, arg1: address, arg2: 0x1::ascii::String, arg3: u256, arg4: &mut 0x2::tx_context::TxContext) {
        send_add_point_req_<T0>(0x43e99dc83d4c41b8db7ae4610f232d59389dc70ec9faadbcd810fc284b4bd561::constant::point_updater(), arg1, arg2, arg3, arg4);
    }

    public fun send_stake_point_req<T0: drop>(arg0: &T0, arg1: 0x1::ascii::String, arg2: u256, arg3: u64, arg4: &0x2::clock::Clock, arg5: &mut 0x2::tx_context::TxContext) {
        let v0 = 0x2::tx_context::sender(arg5);
        send_stake_point_req_<T0>(0x43e99dc83d4c41b8db7ae4610f232d59389dc70ec9faadbcd810fc284b4bd561::constant::point_updater(), v0, arg1, arg2, arg3, arg4, arg5);
    }

    fun send_stake_point_req_<T0: drop>(arg0: address, arg1: address, arg2: 0x1::ascii::String, arg3: u256, arg4: u64, arg5: &0x2::clock::Clock, arg6: &mut 0x2::tx_context::TxContext) {
        let v0 = 0x2::clock::timestamp_ms(arg5);
        let v1 = StakePointRequest<T0>{
            id        : 0x2::object::new(arg6),
            owner     : arg1,
            action    : arg2,
            weight    : arg3,
            duration  : arg4,
            timestamp : v0,
        };
        let v2 = LiquidlinkStakePointEvent<T0>{
            owner     : arg1,
            action    : arg2,
            req       : 0x2::object::id<StakePointRequest<T0>>(&v1),
            weight    : arg3,
            timestamp : v0,
            duration  : arg4,
        };
        0x2::event::emit<LiquidlinkStakePointEvent<T0>>(v2);
        0x2::transfer::transfer<StakePointRequest<T0>>(v1, arg0);
    }

    public fun send_stake_point_req_with_owner<T0: drop>(arg0: &T0, arg1: address, arg2: 0x1::ascii::String, arg3: u256, arg4: u64, arg5: &0x2::clock::Clock, arg6: &mut 0x2::tx_context::TxContext) {
        send_stake_point_req_<T0>(0x43e99dc83d4c41b8db7ae4610f232d59389dc70ec9faadbcd810fc284b4bd561::constant::point_updater(), arg1, arg2, arg3, arg4, arg5, arg6);
    }

    public fun send_sub_point_req<T0: drop>(arg0: &T0, arg1: 0x1::ascii::String, arg2: u256, arg3: &mut 0x2::tx_context::TxContext) {
        let v0 = 0x2::tx_context::sender(arg3);
        send_sub_point_req_<T0>(0x43e99dc83d4c41b8db7ae4610f232d59389dc70ec9faadbcd810fc284b4bd561::constant::point_updater(), v0, arg1, arg2, arg3);
    }

    fun send_sub_point_req_<T0: drop>(arg0: address, arg1: address, arg2: 0x1::ascii::String, arg3: u256, arg4: &mut 0x2::tx_context::TxContext) {
        let v0 = SubPointRequest<T0>{
            id     : 0x2::object::new(arg4),
            owner  : arg1,
            action : arg2,
            value  : arg3,
        };
        let v1 = LiquidlinkSubPointEvent<T0>{
            owner  : arg1,
            action : arg2,
            req    : 0x2::object::id<SubPointRequest<T0>>(&v0),
            value  : arg3,
        };
        0x2::event::emit<LiquidlinkSubPointEvent<T0>>(v1);
        0x2::transfer::transfer<SubPointRequest<T0>>(v0, arg0);
    }

    public fun send_sub_point_req_with_owner<T0: drop>(arg0: &T0, arg1: address, arg2: 0x1::ascii::String, arg3: u256, arg4: &mut 0x2::tx_context::TxContext) {
        send_sub_point_req_<T0>(0x43e99dc83d4c41b8db7ae4610f232d59389dc70ec9faadbcd810fc284b4bd561::constant::point_updater(), arg1, arg2, arg3, arg4);
    }

    public fun send_unstake_point_req<T0: drop>(arg0: &T0, arg1: 0x1::ascii::String, arg2: u256, arg3: u64, arg4: &0x2::clock::Clock, arg5: &mut 0x2::tx_context::TxContext) {
        let v0 = 0x2::tx_context::sender(arg5);
        send_unstake_point_req_<T0>(0x43e99dc83d4c41b8db7ae4610f232d59389dc70ec9faadbcd810fc284b4bd561::constant::point_updater(), v0, arg1, arg2, arg3, arg4, arg5);
    }

    fun send_unstake_point_req_<T0: drop>(arg0: address, arg1: address, arg2: 0x1::ascii::String, arg3: u256, arg4: u64, arg5: &0x2::clock::Clock, arg6: &mut 0x2::tx_context::TxContext) {
        let v0 = 0x2::clock::timestamp_ms(arg5);
        let v1 = UnstakePointRequest<T0>{
            id        : 0x2::object::new(arg6),
            owner     : arg1,
            action    : arg2,
            weight    : arg3,
            duration  : arg4,
            timestamp : v0,
        };
        let v2 = LiquidlinkUnstakePointEvent<T0>{
            owner     : arg1,
            action    : arg2,
            req       : 0x2::object::id<UnstakePointRequest<T0>>(&v1),
            weight    : arg3,
            timestamp : v0,
            duration  : arg4,
        };
        0x2::event::emit<LiquidlinkUnstakePointEvent<T0>>(v2);
        0x2::transfer::transfer<UnstakePointRequest<T0>>(v1, arg0);
    }

    public fun send_unstake_point_req_with_owner<T0: drop>(arg0: &T0, arg1: address, arg2: 0x1::ascii::String, arg3: u256, arg4: u64, arg5: &0x2::clock::Clock, arg6: &mut 0x2::tx_context::TxContext) {
        send_unstake_point_req_<T0>(0x43e99dc83d4c41b8db7ae4610f232d59389dc70ec9faadbcd810fc284b4bd561::constant::point_updater(), arg1, arg2, arg3, arg4, arg5, arg6);
    }

    public fun stake_borrow_action<T0>(arg0: &StakePointRequest<T0>) : 0x1::ascii::String {
        arg0.action
    }

    public fun stake_borrow_duration<T0>(arg0: &StakePointRequest<T0>) : u64 {
        arg0.duration
    }

    public fun stake_borrow_owner<T0>(arg0: &StakePointRequest<T0>) : address {
        arg0.owner
    }

    public fun stake_borrow_timestamp<T0>(arg0: &StakePointRequest<T0>) : u64 {
        arg0.timestamp
    }

    public fun stake_borrow_weight<T0>(arg0: &StakePointRequest<T0>) : u256 {
        arg0.weight
    }

    public(friend) fun stake_point<T0: drop>(arg0: &mut PointDashBoard<T0>, arg1: StakePointRequest<T0>) {
        let StakePointRequest {
            id        : v0,
            owner     : v1,
            action    : v2,
            weight    : v3,
            duration  : v4,
            timestamp : v5,
        } = arg1;
        0x2::object::delete(v0);
        stake_point_internal<T0>(arg0, v1, v2, v3, v5, v4);
    }

    public(friend) fun stake_point_internal<T0: drop>(arg0: &mut PointDashBoard<T0>, arg1: address, arg2: 0x1::ascii::String, arg3: u256, arg4: u64, arg5: u64) {
        init_user_info<T0>(arg0, arg1);
        let v0 = 0x2::table::borrow_mut<address, UserInfo>(&mut arg0.user_infos, arg1);
        if (!0x2::vec_map::contains<0x1::ascii::String, Config>(&v0.configs, &arg2)) {
            let v1 = Config{
                weight      : arg3,
                last_update : arg4,
                duration    : arg5,
            };
            0x2::vec_map::insert<0x1::ascii::String, Config>(&mut v0.configs, arg2, v1);
        } else {
            let v2 = 0x2::vec_map::get_mut<0x1::ascii::String, Config>(&mut v0.configs, &arg2);
            let v3 = checkpoint(v2, arg4, arg3, arg5);
            arg0.total_points = arg0.total_points + v3;
            if (!0x2::vec_map::contains<0x1::ascii::String, u256>(&v0.points, &arg2)) {
                0x2::vec_map::insert<0x1::ascii::String, u256>(&mut v0.points, arg2, v3);
            } else {
                *0x2::vec_map::get_mut<0x1::ascii::String, u256>(&mut v0.points, &arg2) = *0x2::vec_map::get<0x1::ascii::String, u256>(&v0.points, &arg2) + v3;
            };
        };
    }

    public(friend) fun sub_point<T0: drop>(arg0: &mut PointDashBoard<T0>, arg1: SubPointRequest<T0>) {
        let SubPointRequest {
            id     : v0,
            owner  : v1,
            action : v2,
            value  : v3,
        } = arg1;
        let v4 = v2;
        0x2::object::delete(v0);
        init_user_info<T0>(arg0, v1);
        let v5 = 0x2::table::borrow_mut<address, UserInfo>(&mut arg0.user_infos, v1);
        if (0x2::vec_map::contains<0x1::ascii::String, u256>(&v5.points, &v4)) {
            let v6 = *0x2::vec_map::get<0x1::ascii::String, u256>(&v5.points, &v4);
            let v7 = if (v6 <= v3) {
                let (_, _) = 0x2::vec_map::remove<0x1::ascii::String, u256>(&mut v5.points, &v4);
                v6
            } else {
                *0x2::vec_map::get_mut<0x1::ascii::String, u256>(&mut v5.points, &v4) = v6 - v3;
                v3
            };
            arg0.total_points = arg0.total_points - v7;
        };
    }

    public fun total_points<T0>(arg0: &PointDashBoard<T0>) : u256 {
        arg0.total_points
    }

    public fun unstake_borrow_action<T0>(arg0: &UnstakePointRequest<T0>) : 0x1::ascii::String {
        arg0.action
    }

    public fun unstake_borrow_duration<T0>(arg0: &UnstakePointRequest<T0>) : u64 {
        arg0.duration
    }

    public fun unstake_borrow_owner<T0>(arg0: &UnstakePointRequest<T0>) : address {
        arg0.owner
    }

    public fun unstake_borrow_timestamp<T0>(arg0: &UnstakePointRequest<T0>) : u64 {
        arg0.timestamp
    }

    public fun unstake_borrow_weight<T0>(arg0: &UnstakePointRequest<T0>) : u256 {
        arg0.weight
    }

    fun unstake_checkpoint(arg0: &mut UserInfo, arg1: 0x1::ascii::String, arg2: u64, arg3: u256, arg4: u64) : u256 {
        let v0 = 0x2::vec_map::get_mut<0x1::ascii::String, Config>(&mut arg0.configs, &arg1);
        if (arg2 >= v0.last_update) {
            let v2 = v0.weight;
            if (v2 > arg3) {
                v0.weight = v2 - arg3;
                v0.last_update = arg2;
                v0.duration = arg4;
            } else {
                let (_, _) = 0x2::vec_map::remove<0x1::ascii::String, Config>(&mut arg0.configs, &arg1);
            };
            calculate_config_points(v0, arg2)
        } else {
            0
        }
    }

    public(friend) fun unstake_point<T0: drop>(arg0: &mut PointDashBoard<T0>, arg1: UnstakePointRequest<T0>) {
        let UnstakePointRequest {
            id        : v0,
            owner     : v1,
            action    : v2,
            weight    : v3,
            duration  : v4,
            timestamp : v5,
        } = arg1;
        0x2::object::delete(v0);
        unstake_point_internal<T0>(arg0, v1, v2, v3, v5, v4);
    }

    public(friend) fun unstake_point_internal<T0: drop>(arg0: &mut PointDashBoard<T0>, arg1: address, arg2: 0x1::ascii::String, arg3: u256, arg4: u64, arg5: u64) {
        init_user_info<T0>(arg0, arg1);
        let v0 = 0x2::table::borrow_mut<address, UserInfo>(&mut arg0.user_infos, arg1);
        if (0x2::vec_map::contains<0x1::ascii::String, Config>(&v0.configs, &arg2)) {
            let v1 = unstake_checkpoint(v0, arg2, arg4, arg3, arg5);
            arg0.total_points = arg0.total_points + v1;
            if (!0x2::vec_map::contains<0x1::ascii::String, u256>(&v0.points, &arg2)) {
                0x2::vec_map::insert<0x1::ascii::String, u256>(&mut v0.points, arg2, v1);
            } else {
                *0x2::vec_map::get_mut<0x1::ascii::String, u256>(&mut v0.points, &arg2) = *0x2::vec_map::get<0x1::ascii::String, u256>(&v0.points, &arg2) + v1;
            };
        };
    }

    // decompiled from Move bytecode v6
}

